...
- 入力: 42 findings（隔離3回でfindingsと全screen観測が一致）。初期focus未観測 11、初期focusからの孤立 10、状態契約に対する戻る未観測 9、全辺走査でdisabledへ到達 8、focus複合署名がnormal / hover / pressedのいずれかと同一 2、focus可能要素なし 2。shipyardの戻るnavigationは観測済みで、findingに含まれない。
...
到達graphは矢印4方向とTab相当のnext / previousを同じ辺集合へ合成している。このため「矢印だけでは到達不能だがTabなら到達可能」の差は各辺には残る一方、集約したreachable集合では区別しない。後続strict gateで入力方式別の到達性を要求する場合は、action family別のreachable集合へ分離する。
