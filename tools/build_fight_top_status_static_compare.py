...
def _regular_text_width(text: str, size: int) -> float:
    return _font(size, bold=False).getlength(text)
...
def _draw_status_slot(base: Image.Image, draw: ImageDraw.ImageDraw, slot: tuple[float, float, float, float], title: str, body: str) -> None:
    ...
    if title == "AM":
        ...
        _draw_text(draw, (text_x + 29, baseline + 1), body, 22, "#21170f", max_width=max_width - 29)
        return
    if title == "快晴":
        baseline = y0 + h * 0.57
        ...
        _draw_text(draw, (wind_x + 26, baseline), body, 17, "#173f32", max_width=max_width - (wind_x - text_x) - 26)
        return
    if title == "所持金":
        _draw_text(draw, (text_x - 1, y0 + h * 0.57), body, 22, "#21170f", max_width=max_width + 2)
