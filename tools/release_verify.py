...
EXPLAINED_WARNING_RULES = {
    ...
    "save_system": [
        ...
        (re.compile(r"^WARNING: セーブデータを削除できませんでした（コード: 20）。$"), "save verifierの意図的なslot削除失敗fixture"),
...
EXPLAINED_WARNING_TEST_RULES = {
    ...
    ("save_system", "tools/save_namespace_migration_smoke.tscn"): [
...
def warning_summary(output: str, context: str = "all", test: str | None = None) -> dict[str, object]:
    ...
    if test is not None:
        rules += EXPLAINED_WARNING_TEST_RULES.get((context, test), [])
...
def run(command: list[str], cwd: Path, env: dict[str, str], log: Path, context: str = "all", test: str | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "warnings": warning_summary(output, context, test),
        "log": str(log),
