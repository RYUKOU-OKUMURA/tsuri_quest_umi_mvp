...
> 現在は **Pre-RC開発中** です。難易度（E7）は実装済みです。設定・入力・製品外装（E11）、権利確認、対象Mac、署名・公証方針、最終RC検証は未完です。このリポジトリの内容を、そのまま署名・公証済み製品版とみなすことはできません。
...
- 新規セーブ開始時に選べる3難易度（やさしい・ふつう・むずかしい）と、難易度別のファイト・売値・経験値調整
- 竿5種、仕掛け7種、船3種
...
- 31対象へfreeze済みのrelease verifier骨格を固定RCで実行し、最終的な配布物構成と同梱ライセンスを確認
