...
skeleton modeの `skeleton_passed` は、export証跡を指定しなくても30対象の骨格・source検証が成功したことを示します。このとき `export_launch_smoke.tscn` のstatusは `pending_rc_evidence` で、RC証跡による被覆済みを意味しません。RC modeで証跡を消費した場合だけ `delegated_evidence` になります。固定RC検証や最終配布証跡の完了も意味しません。
