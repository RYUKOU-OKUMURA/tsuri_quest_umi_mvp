...
./tools/settings_isolation_self_test.sh
```

次にskeleton modeで、30件の `*_smoke.tscn` / `*_audit.tscn` と、同stemのsceneを持たないscript-only audit 1件を自動列挙し、合計31対象を実行・委譲します。発見集合と `tools/release_test_manifest.txt` は完全一致が必須で、セーブ2件、settings smoke 1件、export 1件の特殊runner 4件は存在・分類とも必須です。settings専用runnerは`tools/settings_smoke.tscn`以外へ割当不可で、run固有の物理HOME・guard sentinel・tokenを付与して実削除UI統合を検証します。manifestはsymlinkでないregular fileを単一readしてparse/hashし、source外manifestを指定した場合も含め、開始・終了hashが一致しなければ失敗します。全体validate、scene/script audit、セーブ検証委譲を実行し、機械可読レポートを生成します。
