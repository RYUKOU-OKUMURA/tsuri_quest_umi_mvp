...
func _draw_key_hint_compact(font: Font, rect: Rect2, key: String, label: String, note: String) -> void:
	var key_w := 34.0 if key.length() <= 1 else 50.0
	var key_rect := Rect2(rect.position + Vector2(8.0, 3.0), Vector2(key_w, 25.0))
	...
	var label_plate := Rect2(label_pos + Vector2(-4.0, -16.0), Vector2(label_w + 9.0, 19.0))
	...
	_draw_text(font, note, Vector2(note_x, key_rect.position.y + 33.0), note_size, Color("#5a4327"), 0)
...
func _hint_key_slots(hint: Rect2) -> Array[Rect2]:
	...
	var slot_y := hint.position.y + 28.0
	var slot_h := 41.0
