...
- [P2] Background is much closer to the reference, but the extracted base still needs final art cleanup.
  ...
  Evidence: the primary background is no longer the generated canyon scene. `tools/build_reference_underwater_background.py` now extracts the reference mockup's left water window, preserving the reference-like left rock mass, bubble rhythm, surface caustics, and darker blue water layers. The main fish, hit burst, line, and lure zones are masked so the runtime kurodai and hit treatment can sit on top without a hard duplicate. A failed first extraction left visible fish remnants and top UI contamination; the current pass removes the top UI strip and replaces the subject area with a clean low-detail blue water field. The old helper overlays initially created a gray film on the new background, so `UnderwaterView` now draws the color-grade, seabed-detail, and ambience textures at lower opacity. The remaining mismatch is that the masked center/right zones are flatter than the reference's hand-authored seabed and right-side vegetation.
  Impact: this is a real quality move toward the target image instead of another procedural polish pass. The water panel now shares the reference's actual art language in the visible left/top zones, and the main fish reads against a calmer background. It is still not a finished background because the central replacement area is clean but slightly plain, and the right side lacks the crisp reference rock/seaweed detail.
  Fix: keep `tools/build_reference_underwater_background.py` as the primary deterministic background build path and keep `UnderwaterView` linear filtering. Next background work should clean up the central/right replacement art or provide a fully authored replacement background; do not return to adding small particles as the main fix.
...
- None blocking. The next highest-value pass remains final background art cleanup. The reference-derived background is now a better direction than the earlier generated base, but its central/right masked area still needs authored detail or a cleaner replacement.
