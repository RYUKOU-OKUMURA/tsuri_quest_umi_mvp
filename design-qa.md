...
- Added `tools/build_reference_underwater_background.py`, which builds `underwater_battle_bg.png` from the reference mockup's water window instead of continuing to polish the generated background as the primary source. The pass masks the main fish, hit burst, line, and lure zones, fills them with a low-detail blue water field, expands the crop to the existing 1672x941 texture slot, and harmonizes the result for use under the runtime fish/HUD.
...
- Added a deterministic masked-area detail pass to `tools/build_reference_underwater_background.py`. It bakes subtle seabed caustic lines, lower/right rock silhouettes, taller seaweed/coral clusters, small stones, and faint right-side fish silhouettes into `underwater_battle_bg.png` so the previously flat blue mask area no longer reads as an empty wall behind the runtime fish.
- Added a reference-pixel side-detail pass to `tools/build_reference_underwater_background.py`, softly mirroring the reference crop's left rock/seabed texture into the lower-right replacement area. The first version made a visible rectangular patch; the current mask moves the patch rightward and uses stronger feathering/lower opacity so it reads as background texture rather than a pasted tile.
- Added a second reference-pixel right-reef pass to `tools/build_reference_underwater_background.py`. It crops the authored right-side reef from the full reference mockup, extracts mostly rock/seaweed/bubble detail with a content mask, and composites it after the depth/detail pass so the far-right background has more reference-derived structure without bringing back the lure or obvious rectangular water block.
...
- [P2] Right panel is structurally better but still below reference card quality.
  ...
  Evidence: the panel is now wider, starts at the top of the screen beside the status cards, uses a dedicated paper-backed kurodai portrait instead of the runtime swimming sheet, and the previous dark portrait rectangle regression is gone. The fish-card title row now sits on an opaque paper plaque, so `No.028 / クロダイ / レア` reads clearly like the reference instead of sinking into a dark band. Moving the status bar into the left column gives the right panel more vertical room, and the 620x330 `kurodai_card_portrait.png` ratio now fits the runtime fish window better than the previous 720x330 source, making the fish read larger and less like a shrunken document thumbnail. The latest lower-card runtime pass makes the action text larger, reduces the card icon footprint, condenses the tackle card into two readable lines, and fixes the tackle-card baseline after an initial too-high placement. The cards are less shrunken, but still feel more mechanically framed than the reference, especially around small-text optical quality and the authored paper-card finish.
  ...
  Fix: keep the larger dedicated fish-card portrait, wider panel, paper title plaque, card-specific lower icons, separated lower-card wells, shorter lower-card copy, stronger lower-card runtime text, and the two-line tackle copy. Next, move to background density/final UI frame art unless the right panel regresses.
...
- [P2] Background is much closer to the reference, but the extracted base still needs final art cleanup.
  ...
  Evidence: the primary background is no longer the generated canyon scene. `tools/build_reference_underwater_background.py` now extracts the reference mockup's left water window, preserving the reference-like left rock mass, bubble rhythm, surface caustics, and darker blue water layers. The main fish, hit burst, line, and lure zones are masked so the runtime kurodai and hit treatment can sit on top without a hard duplicate. A failed first extraction left visible fish remnants and top UI contamination; the current pass removes the top UI strip and replaces the subject area with a clean blue water field. The old helper overlays initially created a gray film on the new background, so `UnderwaterView` now draws the color-grade, seabed-detail, and ambience textures at lower opacity. The background-builder now adds baked seabed caustics, lower/right rock silhouettes, taller seaweed/coral clusters, small stones, faint fish silhouettes, a softly feathered mirrored reference-rock texture, and a separate content-masked right-reef patch from the full reference mockup. In `/tmp/tsuri_fight_compare.png`, the implementation's right edge now has more rock/seaweed structure than the previous flat blue wall, but it remains softer and farther back than the reference's crisp right rock pile and vegetation.
  Impact: this is a real quality move toward the target image instead of another procedural polish pass. The water panel now shares the reference's actual art language in the visible left/top zones, the main fish reads against a calmer background, and the bottom/right of the scene is less empty. It is still not a finished background because the added right-side detail improves texture but does not yet reach the reference's authored rock/seaweed density or foreground contrast.
  Fix: keep `tools/build_reference_underwater_background.py` as the primary deterministic background build path and keep UnderwaterView linear filtering/lower overlay opacity. Next background work should use a stronger authored raster replacement for the central/right masked art; do not keep trying to solve the remaining gap with small particles or overly opaque rectangular reference patches.
...
- None blocking. The next highest-value pass remains final background art cleanup. The reference-derived background and baked masked-area detail are a better direction than the earlier generated base, but the central/right area still needs a more authored replacement to reach the reference's hand-painted density.
