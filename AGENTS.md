...
- 合格済みのfreeze値（`docs/qa/<screen>_qa.md` のfreeze表記載）は原則動かさない。再オープンは `docs/19` §0/§1.1/§6.4 の3条件（P1破綻の再発 / ユーザー承認済みの参照・UX方向更新 / 高視線領域の測定可能な収束差分）に限定し、対象値・不動値・同一状態before/after・新採用値をQAへ記録する。
