...
今回の実装では、港から独立した `船着き場` 画面へ遷移し、そこで船を購入できる。釣具店は竿の購入・装備に戻し、船購入の責務を持たない。

`船着き場` 画面は次を表示する。

- 左：船3種のカード一覧、購入状態、価格、船名、到達範囲
- 中央：選択中の船、購入状態、価格、不足額、航行ランク、購入ボタン
- 右：現在の出航範囲と、選択船購入後に広がる航路/到達範囲
- 下：購入結果、所持金不足、港へ戻る導線

背景画像の空欄フレームは、状態バッジ、価格バッジ、船名帯、購入前/購入後の航路レジェンドで上書きし、未完成の空欄に見せない。

釣り場選択では、右詳細と `ここで釣る` 実行時に船不足を止める。
