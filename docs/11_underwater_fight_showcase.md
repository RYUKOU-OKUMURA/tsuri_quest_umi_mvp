...
1. `sidebar_frame.png` は行動/タックル下段カード拡張、内部罫線、角ブラケット追加まで完了。次は右パネル本文と見出しの光学サイズを参照画像へ寄せる。
2. `fight_hud_frame.png` の上段深度プレートと角度付きセパレーターは強化済み。次は上段の色バランス、ゲージラベル間隔、深度タイトル/数値のリズムだけを詰める。
3. 文字サイズの最終調整を行う。`MPLUS1p-Bold.ttf` で太さは改善したが、右パネル本文、HUDの値表示、キー表示は再チューニングする。
4. ヒット演出は HUD 形状が固まったため、下端の重なりと明度を最終調整する。
