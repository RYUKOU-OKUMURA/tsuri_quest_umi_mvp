...
- `FightHud` は画面全幅ではなく左カラム内に置き、`fight_hud_frame.png` の上にゲージ色、現在位置、深度、操作文字を重ねる。`fight_hud_frame.png` は 2048x456 の二段操作盤として作っているため、実表示では 206px 前後の高さを確保し、縦に潰れた細い帯として見せない。214px まで上げると水中窓が縦に詰まり、主役魚/ヒット領域の看板性を削るため、現行は 206px を上限寄りの基準にする。ゲージは18分割の細かいセグメントを基準にし、塗り済みセグメントだけでなく、薄い未充填セグメント、上ハイライト、下影、テンション位置マーカー影を描き、粗い矩形の羅列ではなく操作盤に埋まったメーターとして見せる。
...
- `FightSidebar` の行動/タックル小カードは `fight_action_card_icon.png` / `fight_tackle_card_icon.png` を優先する。共通アイコンシートは大きな装飾を含むため、右下段カードではカード専用にトリミングした透明切り抜き素材を使う。`sidebar_frame.png` がある本番寄り経路では、下段カードは一枚の紙面として焼き込み、Godot側で追加の本文パネルを描かない。右下段カードの本文は、長文情報をそのまま詰め込まず、アイコン横で読める短い行に圧縮する。特にタックルカードは3行の小文字を避け、ロッド行と `糸3号・チヌ針` の2行にまとめて可読サイズを優先する。行動カード本文は15px前後、タックル本文は19px前後を基準にし、小さな注釈ではなくカードに印字された説明として読ませる。行動カードの2行メッセージは、下段のガイド線や枠に触れないよう、本文紙面を少し高めから使い、行間を詰めてカード内の下余白を残す。小カードでは本文サイズとベースラインを優先するが、タックルカードだけは参照のロッド/リールの存在感に近づけるため、透明切り抜きアイコンを 64x58px 程度まで大きく使い、右側の道具ビジュアルとして読ませる。下段カード内部で本文ウェルとアイコンウェルを分けすぎるとフォーム部品に見えるため、枠線・角金具・内影は弱い一体紙面の装飾に留める。
...
1. `sidebar_frame.png` は右パネル幅拡張、魚カード用 `kurodai_card_portrait.png` 分離、魚カード見出しの紙プラーク化、紙面の内部構造追加、行動文の意味改行、行動/タックル下段カード専用アイコン追加、下段カード本文の短縮、余白調整まで完了。`kurodai_card_portrait.png` は魚の占有率を上げ、カード内の空白感を減らした。魚カード下部は、素材側の強いガイド線を外し、Godot側の `推定 44.2 cm` と生態メモが線に切られない状態へ調整済み。生態メモは通常フォントと短い `エサ：オキアミ・カニ` 表記にして、太字小文字が暗い帯に見える問題を避けている。最新パスでは生態メモを 14px 相当に上げ、緑ドットも少し強くして、右カード下部が小さな注釈ではなく印字情報として読めるようにした。行動/タックル下段カードは、見出し帯を浅くし、紙本文ウェルを縦に広げ、アイコン占有と内部ガイド線を抑え、Godot側の二重本文パネルを外し、タックル本文を2行に圧縮し、紙スロット上端/下端に干渉しないベースラインへ調整済み。最新パスでは、下段カードのアイコン枠/本文枠の分離をやめ、専用アイコンを透明背景の切り抜き素材に再生成して、一枚の紙カード上にアイコンと本文が印字されている見え方へ寄せた。さらに action アイコンは 50px、tackle アイコンは 64x58px へ上げ、特にタックルカード右側が小さな針アイコンではなく道具ビジュアルとして読めるようにした。行動本文は15px、タックル本文は19pxの二行印字を維持する。次は全体比較で右パネルが再び浮く場合だけ小さく調整する。
2. `fight_hud_frame.png` の上段は深度プレート強化、暗色化、HUDアイコン縮小/低透過化、右ラベル余白調整、18分割メーター化、未充填セグメント表示とハイライト/影追加まで完了。下段はエサ/操作/メニューの紙タイトル帯、本文スロット、濃紺メニュー行、操作ヒントの3スロット化、A/B/LRキー配置の整列、キーキャップ/ラベルの拡大と低めの再配置、キーキャップの暗い外枠/金内枠/上ハイライト、操作ヒント幅の拡張、キー下の短い補足文追加まで完了。最新パスではエサ/メニュー比率を維持したまま、焼き込み操作スロットを runtime スロットの高さに合わせてさらに厚くし、A/Bキーキャップ、主ラベル、補足文を一段大きくした。L/Rだけはキー幅と文字サイズを少し抑え、`調整 / テンション調整` が第三スロット内に収まるようにしている。`FishingScreen` と `FightHud` の表示最小高は 206px に上げ済みで、2048x456 のフレーム比率を極端に縦圧縮しない状態にしている。214px 試行は水中窓を詰めすぎたため採用しない。次は最終比較でまだ機械的に見える場合、HUDフレーム素材そのものの比率と小文字の光学サイズを調整する。
