...
|---|---|---|---|
| `assets/showcase/underwater/underwater_battle_bg.png` | 水中背景 | `tools/source_assets/underwater_battle_bg_source.png` を `tools/enhance_underwater_battle_bg.py` で後処理 | 16:9、暗部と主役魚のコントラストを確保、中心の魚シルエットを邪魔しない |
| `tools/source_assets/underwater_battle_bg_source.png` | 水中背景の元画像 | 本番寄り背景PNGの保存元 | Godotインポート対象外、後処理を重ねがけしないための決定的ソース |
