...
1. `sidebar_frame.png` は行動/タックル下段カードを拡張済み。次は枠の角飾り、内部罫線、右パネル本文の文字サイズを参照画像へ寄せる。
2. `fight_hud_frame.png` の上段深度プレートと角度付きセパレーターは強化済み。次は右パネル後に、上段の色バランス、ゲージラベル間隔、深度タイトル/数値のリズムだけを詰める。
3. 文字サイズの最終調整を行う。`MPLUS1p-Bold.ttf` で太さは改善したが、右パネル本文、HUDの値表示、キー表示は枠再調整後に再チューニングする。
