...
| `tools/build_reference_underwater_background.py` | 水中背景ビルダー | 参照水中窓全体クロップ、主役/ヒット/糸/ルアーの広域マスク、水色面補完、参照由来の中央水底/岩場テクスチャ、魚群/水中テクスチャ、上げた海底棚、中央床反射、遠景魚影、小泡、光芒の再合成、波状/不定形アルファ、キャンバス拡張、色調整 | 中間PNGを残さず、最終背景だけを決定的に再生成する |
...
- `FightHud` は画面全幅ではなく左カラム内に置き、`fight_hud_frame.png` の上にゲージ色、現在位置、深度、操作文字を重ねる。ゲージは18分割の細かいセグメントを基準にし、塗り済みセグメントだけでなく、薄い未充填セグメント、上ハイライト、下影、テンション位置マーカー影を描き、粗い矩形の羅列ではなく操作盤に埋まったメーターとして見せる。
...
- `FightSidebar` の行動/タックル小カードは `fight_action_card_icon.png` / `fight_tackle_card_icon.png` を優先する。共通アイコンシートは大きな装飾を含むため、右下段カードではカード専用にトリミングした素材を使う。`sidebar_frame.png` がある本番寄り経路では、下段本文の紙ウェルは素材側に焼き込まれているため、Godot側で追加の本文パネルを描かない。右下段カードの本文は、長文情報をそのまま詰め込まず、アイコン横で読める短い行に圧縮する。特にタックルカードは3行の小文字を避け、ロッド行と `糸3号・チヌ針` の2行にまとめて可読サイズを優先する。行動カード本文は14px前後を基準にし、小さな注釈ではなくカードに印字された説明として読ませる。行動カードの2行メッセージは、下段のガイド線や枠に触れないよう、タイトル/本文ブロックをやや上に置き、行間を詰めてカード内の下余白を残す。小カードではアイコンを本文より主張させず、本文サイズとベースラインを優先して、縮小UIではなくカードに印字された情報として見せる。
- `FightStatusBar`、`FightHud`、`FightSidebar`、`UnderwaterView` のヒット文字は `src/ui/fight_fonts.gd` から水中ファイト専用の太字フォントを読み込む。
- `FightHud` の下段は、`fight_hud_frame.png` 側にエサ/操作/メニューの紙タイトル帯、本文スロット、濃紺メニュー行を焼き込み、Godot側でエサ数、A/B/LRのキーキャップ、短いラベル、控えめな補足文を重ねる。PNG側の装飾枠と実コードの座標がずれると、文字が暗い盤面に沈んでデバッグUIに見えるため、フレーム生成とGodot描画の座標を必ず合わせる。操作ヒントはメニュー幅を取りすぎず、参照のように横へ広く読ませる。キーキャップ、主ラベル、補足文は紙スロット内に収め、薄い説明テキストではなくカードに印字された操作要素として見せる。A/B/LR のキーキャップは暗い外枠、金の内枠、薄い上ハイライトを持つ小さな物理ボタンとして描き、単なる文字ラベルに戻さない。操作ラベルは紙スロットの罫線と重なりやすいため、ラベル背後に小さな紙プラークを描き、罫線が文字を横切らないようにする。
...
1. `sidebar_frame.png` は右パネル幅拡張、魚カード用 `kurodai_card_portrait.png` 分離、魚カード見出しの紙プラーク化、紙面の内部構造追加、行動文の意味改行、行動/タックル下段カード専用アイコン追加、下段カード本文の短縮、余白調整、アイコン枠/本文プラーク分離まで完了。`kurodai_card_portrait.png` は魚の占有率を上げ、カード内の空白感を減らした。魚カード下部は、素材側の強いガイド線を外し、Godot側の `推定 44.2 cm` と生態メモが線に切られない状態へ調整済み。生態メモは通常フォントと短い `エサ：オキアミ・カニ` 表記にして、太字小文字が暗い帯に見える問題を避けている。行動/タックル下段カードは、見出し帯を浅くし、紙本文ウェルを縦に広げ、アイコン占有と内部ガイド線を抑え、Godot側の二重本文パネルを外し、タックル本文を2行に圧縮し、紙スロット上端/下端に干渉しないベースラインへ調整済み。最新パスでは下段の白い本文箱を、紙テクスチャ、内影、控えめな角金具、短い低透過ガイド線を持つリセスに差し替え、フォーム/デバッグUI感を少し弱めた。さらに行動カードのアイコンを少し小さくし、行動タイトル/本文を大きくし、タックル本文を `ロッド：...` と `糸3号・チヌ針` に短縮して本文幅を広げた。次は全体比較で右パネルが再び浮く場合だけ小さく調整する。
2. `fight_hud_frame.png` の上段は深度プレート強化、暗色化、HUDアイコン縮小/低透過化、右ラベル余白調整、18分割メーター化、未充填セグメント表示とハイライト/影追加まで完了。下段はエサ/操作/メニューの紙タイトル帯、本文スロット、濃紺メニュー行、操作ヒントの3スロット化、A/B/LRキー配置の整列、キーキャップ/ラベルの拡大と低めの再配置、キーキャップの暗い外枠/金内枠/上ハイライト、ラベル背後の小さな紙プラーク追加、操作ヒント幅の拡張、キー下の短い補足文追加まで完了。次は最終比較でまだ機械的に見える場合だけ全体比率と小文字の詰めを行う。
3. 上部ステータスの地点カードは参照に合わせてアイコンなし中央寄せに修正済み。`top_status_frame.png` は紙カード内枠、角金具、濃紺地点カードの内装追加まで完了。上部アイコン群は `top_status_icon_sheet.png` に分離し、時計/太陽/風/コインが潰れない状態まで改善済み。`FightStatusBar` は左ファイトカラム内へ移動し、右パネルヘッダーが上端から始まる構造に修正済み。カード比率は天候/所持金を広げ、地点カードを締める方向へ調整済み。文字ベースラインと数値の光学サイズも調整し、所持金は小ラベルを外して金額を大きく読ませる状態まで改善済み。画面外側の root margin と主要 container separation も詰め、黒い外側ガターを減らして参照の画面密度に近づけている。最新パスでは `top_status_frame.png` と `FightStatusBar._slot_rects()` のスロット境界を維持しつつ、時計、天候、所持金、水深の主文字と上部アイコンを少し大きくし、`AM` と時刻のまとまりを強めた。次に詰めるなら、背景または右パネルの本番素材差分が埋まったあとに最終比較で微調整する。
...
5. 泡、光粒、魚影は `underwater_foreground_ambience.png` と `UnderwaterView` の補助光粒として追加済み。背景の均一な明るさは `underwater_color_grade.png` で少し締め、海底/左右の密度は `underwater_seabed_detail.png` で補っている。`underwater_battle_bg.png` 本体は `tools/build_reference_underwater_background.py` でリファレンス水中窓全体から抽出し、主役魚/ヒット/糸/ルアー部分をマスク補完して再生成済み。左右岩場、海藻、泡、遠景魚、海底光は参照画像の実ピクセルを残すため、旧生成背景や右端だけのパッチより密度が高い。中央補完面には参照由来の左右海底/水中テクスチャ、低コントラスト泡、薄い水底光、参照水面光を伸ばした光パッチ、下部だけに抑えた水底/岩場断片、遠景魚影、中央寄りの小泡、上からの光芒、参照由来の魚群/水中テクスチャ、少し上げた海底棚、低アルファの中央床反射を戻し、丸い塗り跡と空白感を弱めている。補完パッチは波状の縦フェードと不定形マスクでアルファを絞り、横長の塗り面に見える問題を抑えている。`underwater_foreground_ambience.png` には中央の小さな泡列と低アルファの水底光も足し、主役魚やヒット文字を邪魔せずに中央水域の空白感を少し減らしている。参照抽出背景に合わせ、`UnderwaterView` 側では色調整/海底補助/前景 ambience の opacity を下げて灰色の膜を避け、主役魚背後の大きなシアン円形オーラも外した。`UnderwaterView` は LINEAR フィルタで showcase テクスチャを描画し、背景の縮小時の硬さを抑える。次に背景を詰める場合は、中央マスク領域を最終ラスタとして描き込み、参照中央の水底/水中密度へ近づける。
