...
- `UnderwaterView` は `underwater_battle_bg.png` の上に、遠景魚群、泡柱、光粒の薄いアンビエンスレイヤーを重ねる。これは背景PNGの不足を完全に代替するものではなく、主役魚を邪魔しない密度補助として扱う。
...
5. 泡、光粒、魚影は `UnderwaterView` のアンビエンスレイヤーとして追加済み。次に背景を詰める場合は、`underwater_battle_bg.png` 自体の岩場・海藻・海底ディテールを強めるか、前景用の専用ラスタ泡/光素材に分離する。
