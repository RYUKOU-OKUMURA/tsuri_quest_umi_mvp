...
- `FishingScreen` の水中ファイト時レイアウトは、通常アプリ画面より外側余白とカラム間隔を狭くする。参照画像はステータス、海中窓、HUD、右パネルが画面端近くまで詰まっており、黒い外側ガターが広いと看板画面の密度が落ちるため、root margin と主要 container separation は控えめにする。
