...
- `UnderwaterView` は主役魚とヒット演出のスケールを参照画像寄りに抑え、深度目盛りは背景に馴染む低コントラスト表示にする。魚の背後には大きな円形オーラを置かず、必要な場合も小さく薄い影だけにする。背景・魚・ヒット素材の上からコード描画の半透明円を重ねると、参照にはない合成跡として読まれるため。
