...
- `FightHud` の下段は、`fight_hud_frame.png` 側にエサ/操作/メニューの紙タイトル帯、本文スロット、濃紺メニュー行を焼き込み、Godot側でエサ数とA/B/LRのキーキャップと短いラベルだけを重ねる。PNG側の装飾枠と実コードの座標がずれると、文字が暗い盤面に沈んでデバッグUIに見えるため、フレーム生成とGodot描画の座標を必ず合わせる。操作ヒントはキーキャップとラベルを紙スロット内でやや大きく、低めに置き、薄い説明テキストではなくカードに印字された操作要素として見せる。A/B/LR のキーキャップは暗い外枠、金の内枠、薄い上ハイライトを持つ小さな物理ボタンとして描き、単なる文字ラベルに戻さない。操作ラベルは紙スロットの罫線と重なりやすいため、ラベル背後に小さな紙プラークを描き、罫線が文字を横切らないようにする。
...
2. `fight_hud_frame.png` の上段は深度プレート強化、暗色化、HUDアイコン縮小/低透過化、右ラベル余白調整、18分割メーター化、未充填セグメント表示とハイライト/影追加まで完了。下段はエサ/操作/メニューの紙タイトル帯、本文スロット、濃紺メニュー行、操作ヒントの3スロット化、A/B/LRキー配置の整列、キーキャップ/ラベルの拡大と低めの再配置、キーキャップの暗い外枠/金内枠/上ハイライト、ラベル背後の小さな紙プラーク追加まで完了。次は最終比較でまだ機械的に見える場合だけ全体比率と小文字の詰めを行う。
