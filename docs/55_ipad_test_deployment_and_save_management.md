...
状態: **初回実機導入spike完了（表示・入力・セーブQAは継続）**
...
| 質問 | 現在の回答 |
|---|---|
| iPadへテスト導入できるか | **可能**。Personal Team署名のdebug buildを実機へinstallし、端末上で起動・プロセス継続を確認済み |
| 今すぐ入れて通常プレイできるか | **アプリは起動可能**。通常プレイ可否は実機表示・入力・セーブQAの完了まで保証しない |
...
| iPad接続 | USB接続、信頼、Developer Mode、pairing、install、初回起動完了 | 表示・入力QAを継続 |
...
したがって、iOS export / build / install / launchの最小spikeは通過した。次は**タイトル表示を目視確認し、入力とセーブ更新回帰を同じ実機で確認すること**である。
...
### 7-3. 2026-07-18 初回導入spike結果

| 項目 | 結果 |
|---|---|
| 環境 | iPad Air（第3世代）/ iPadOS 26.5.2 / Godot 4.7 / Xcode 26.6 |
| source commit | `9cde8e2` |
| 署名 | Personal Team / Apple Development / managed provisioning |
| Bundle ID | `net.physical-balance-lab.tsuri-quest-umi` |
| Godot export | `iOS iPad Debug`からXcode project生成成功 |
| Xcode build | Debug / arm64 / 実機destinationで成功 |
| install | 成功。初回のみ端末側でデベロッパAppを信頼 |
| launch | 成功。起動直後に対象プロセスの継続動作を確認 |
| 判定 | 導入経路は通過。`IPAD-INSTALL-01`のタイトル目視と、入力・表示・性能・セーブ項目は未完 |
