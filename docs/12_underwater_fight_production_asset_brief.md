...
現行採用ソースは、右向き全身クロダイを純マゼンタ背景で生成した `tools/source_assets/kurodai_final_art_source.png` である。最新採用候補は、前ソースより背中、黒帯、背びれ、鱗密度が締まり、`/tmp/tsuri_fish_asset_contact.png`、`/tmp/tsuri_fish_hit_focus.png`、`/tmp/tsuri_fight_compare.png` で確認済みである。口元はやや強いが、以前拒否した暗い2枚目候補ほど頭部が支配的ではなく、全画面では魚の重さと参照寄りの密度が少し上がるため採用する。参照画像の魚よりまだ滑らかな描画ではあるため、次に更新する場合は、同じレイアウトに載せた全画面比較でこの現行採用ソースを明確に上回る候補だけを使う。
