...
### 0.2 2026-07-18 権利者・macOS署名方針追補

- 製品コードの個人権利者名について、所有者回答 `奥村 龍晃` または `OKUMURA RYUKOU` を受領した。公開表記は日本語＋ローマ字併記 `奥村 龍晃（OKUMURA RYUKOU）` を推奨するが、exact表記は未確定。exact表記、非公開原本を確認したU-05 canonical attestation、証拠index同期が未保存のため、`LICENSE.md` placeholderとRIGHTS-01A markerはpendingを維持し、全条件の確定と同時に置換する
- 現行 `assets/icon.svg` は製品不採用と確定した。新規製品iconを制作中であり、差し替え製品と権利証拠が台帳化されるまでU-04およびREL-03を完了扱いにしない
- 初回itch.io macOS Universal版は、`Developer ID Application`署名、Hardened Runtime、`notarytool`公証、stapleをRC必須ゲートとする。2026-07-18時点でApple Developer Program未加入、証明書未発行のため外部pendingであり、未署名・未公証exportは最小spike / QAに限って使用する
...
- 2026-07-18: 製品コード権利者名候補の所有者回答と日本語＋ローマ字併記の推奨、現行icon不採用、Apple Developer Program未加入、およびitch.io macOS版RCのDeveloper ID署名・Hardened Runtime・公証・staple必須方針を追補。未確定の権利者exact表記、U-05 canonical attestation、差し替えicon、署名証拠はpendingを維持
