...
状態: **superseded** — 正本は `docs/30_v2_expansion_overview.md`（台帳・共通仕様）＋ `docs/v2/E*.md`（フェーズ別実装仕様）。本docの内容はフェーズ別docへ分割済みで、E0（10→30 は 10→50 へ改訂）・E4（サメ2種案は10種ロスターへ改訂）・称号カタログ・fish_no 採番は**本docの記載が古い**。実装時に本docを参照しないこと
