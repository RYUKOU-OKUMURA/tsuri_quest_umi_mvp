...
`reference/cooking_flow/01_cook_select_concept.png` の左カラムは、所持魚が大きい魚絵、魚名、数量で横並びになり、料理素材を選ぶ導線として最初に読める。途中実装では魚行が `status_card_frame.png` の流用で、下部ステータスカードと同じ素材言語に寄っていたため、今回 `fish_row_frame.png` を追加し、`FishRowAji` / `FishRowSaba` / `FishRowMadai` / `FishRowKasago` / `FishRowHirame` / `FishRowKawahagi` として接続する。左の選択ガター、中央の魚アート置き場、右の数量置き場を素材側へ持たせ、魚一覧を単なるデータ行ではなく調理素材カードとして扱う。content/layout auditでは `fish_row_frame.png` の寸法、6行魚リスト、表示中魚行の最小サイズを検出する。
...
- 追記: 右詳細の最終調整では `SelectedDishFeatureImage` の縦量を抑え、`CookDetailMaterialRow` / `CookDetailExpRow` / `CookDetailEffectRow` と `CookButton` の高さ・文字サイズを優先した。参照01との差分確認では、料理写真の面積よりも「材料→EXP→次回効果→調理する」の読み順が一画面内で強く見えることを完成基準にする。
- 追記: `CookActionRunway` 上段の `調理後は食事結果へ` は裸テキストだとフレーム線と混ざるため、小さな濃色バッジとして固定する。調理ボタン直前の説明は薄い注釈ではなく、右詳細のラベル帯と同じ視覚言語で読ませる。
...
- 追記: `PrepSummaryCardLevel` は `Lv.4 130/150` の一行テキストから、参照01に近い `プレイヤーLv.`、大きなレベル数字、青いミニEXPバー、`130/150` 表示の構成へ変更する。`PrepSummaryLevelGauge` / `PrepSummaryLevelExpText` をcontent/layout audit対象にし、下部バーのプレイヤーカードが文字だけの仮カードへ戻らないよう固定する。
...
- 追記: `CurrentPrepTitle` は単独ラベルではなく `CurrentPrepTitleSlot` 内へ移し、通常の準備表示では小さな `CurrentPrepTitleIcon` と余白を添える。下部バー左端が濃色の空き箱に見えないよう、プレイヤーの現在準備を示す見出しポケットとして固定する。食事結果など長い文言を出す状態ではアイコンを隠し、タイトル幅を確保する。
...
- 追記: 魚行は調理専用の小アイコンではなく、`assets/showcase/fish/*_card_portrait.png` を `FightFishAssets.card_portrait_path()` 経由で優先し、水中ファイト基準の魚アート密度をリスト内でも使う。数量欄は右側に確保し、`× 12 匹` などが折れずに読める状態を維持する。表示順は調理画面用に `アジ → サバ → マダイ → カサゴ → ヒラメ → カワハギ` を優先し、赤身・根魚・平たい魚の見た目の幅もcontent/layout audit対象にして、6行密度を維持する。
...
- 追記: 左食事シーンは人物PNG、背景PNG、食卓前景がそれぞれ読める一方、まだ別パーツを重ねた印象が残っていたため、`MealSceneTableBridge` を `MealResultSceneArt` と人物/料理の間へ追加した。下半分に共通のテーブル面、前縁ハイライト、人物と料理の共有影、暖色グローを描き、人物・皿・窓辺が同じ食卓空間に乗っているようにする。EXP_GAINでは非表示にし、MEAL_RESULTだけの一枚絵補強としてcontent/layout auditで固定する。
- 追記: 参照02は皿から立つ湯気が人物と料理の間に広がり、食べた直後の香りが画面左側をまとめている。現行では皿そのものの湯気はあるが空間全体の香りが弱かったため、`MealSceneTableBridge` に薄い共有湯気アークを追加した。前景の人物/料理を隠さない低アルファに留め、左シーンの一枚絵感だけを補強する。
- 追記: 実スクショでは左の大皿がカード右端へ寄って見え、人物側の床面と料理側の皿がまだ別パーツに見えやすかったため、`MealTableSpread` の食卓前景スケールを少し抑えて左へ寄せ、`MealSceneTableBridge` に人物と皿をまたぐ暖色の共有グローとテーブル前縁ラインを追加した。参照02のように人物、皿、湯気が同じ食卓面に乗っている読みを補強し、料理の迫力は保ったまま右端の窮屈さを減らす。
- 追記: 参照02の左食事シーンは窓辺の夕方光が人物、皿、テーブルをまたいでいて、額縁内の絵ではなく食卓ステージとして読める。現行は背景/人物/料理の接続は改善したが、全体が暗いカード内に沈みやすかったため、`MealSceneTableBridge` に窓から食卓へ落ちる暖色ビームと、人物側から皿側へ伸びる共通ハイライトを追加した。`MealSceneActor` / `MealTableSpread` の上には乗せず背面だけで効かせ、前景の可読性を保ちながら左シーンの一枚絵感を補強する。
- 追記: 最新スクショでは左食事シーンの構図は整っているが、人物と皿の上に同じ部屋光が乗っておらず、まだ暗い額縁内に個別パーツを置いた印象が残った。MEAL_RESULTだけ `MealSceneForegroundGlow` を追加し、人物と大皿の上に薄いランタン光、テーブル前縁のリムライト、前景湯気、金スパークを重ねる。新しい文言やカード枠は増やさず、左シーンを参照02のような食卓ステージへ寄せる。EXP_GAINでは非表示に戻し、左カードを料理からEXPへ流れる説明役へ戻す。
- 追記: 最新スクショでは左食事シーン内の光と湯気はまとまってきたが、外枠が厚い暗色カードとして残り、参照02のような開いた食卓ステージより「額縁内の絵」に見えやすい。MEAL_RESULT時だけ `MealSceneCard` の塗りと枠線を弱め、影も軽くして背景/人物/料理を一枚絵として読ませる。EXP_GAINでは従来の強い左カードへ戻し、料理からEXPへ流れる説明カードとしての役割を維持する。
- 追記: 追加比較で `meal_result_scene_art_v2.png` 自体は人物、料理、窓辺、ランタン、食卓小物まで揃った一枚絵として参照02に近い一方、実スクショではその上の `MealSceneActor` / `MealTableSpread` が強く、低解像度の重ね絵に見えやすかった。MEAL_RESULT時は `MealResultSceneArt` の表示アルファを上げ、`MealSceneActor` / `MealTableSpread` / `MealSceneTableBridge` は可視ノード契約を残したまま低アルファの補助前景へ下げる。EXP_GAINでは従来どおり料理ソースカードとして前景料理を使い、状態分離を維持する。
- 追記: 直近スクショでは一枚絵化は進んだが、`MealSceneCard` の内容余白と薄い枠がまだ食卓をカード内に閉じ込めていた。MEAL_RESULT時だけ `MealSceneCard` の内容余白を詰め、塗り/枠線/影をさらに弱めて、左側の人物・料理・窓辺が参照02のように開いた食卓ステージとして読めるようにする。EXP_GAINでは説明カードとしての強い枠を維持する。
- 追記: 最新比較では `meal_result_scene_art_v2.png` 単体は一枚絵として成立しているが、実画面では `MealSceneCard` の薄い外枠と補助前景の線がまだ額縁感を残していた。MEAL_RESULT時だけ `MealSceneCard` を透明に近い1px枠へ下げ、内容余白を1pxにし、`MealResultSceneArt` の表示アルファを上げる。`MealSceneActor` / `MealTableSpread` / `MealSceneTableBridge` はノード契約を維持しつつさらに低アルファ化し、左側は説明カードではなく食卓の一枚絵として読ませる。EXP_GAINでは従来の強い左カードへ戻す。
- 追記: 最新スクショでは左シーンの人物がまだ背景内の小キャラに寄って見え、参照02の「食卓で食べている主役」ほど前に出ていなかった。MEAL_RESULT時だけ `MealSceneActor` の表示幅を広げ、`MealTableSpread` を少し抑えて総幅を保つことで、人物と皿の主従を近づける。EXP_GAINでは従来の俳優サイズへ戻し、左カードが料理からEXPへ流れる説明役に戻る状態分離を維持する。
- 追記: 人物の存在感は上がったが、左食卓前景の `meal_table_spread.png` は魚の焼き目、塩、薬味、皿の質感が右の大判料理より軽く、参照02の「料理そのものが主役の食卓」にはまだ届かない。次パスでは `dish_feature_aji_shioyaki.png` の高密度な皿/焼き魚素材を透明の食卓前景へ再構成し、木目の矩形カードに戻らないよう楕円皿マスク、テーブル影、椀/湯気/塩粒だけを残す。`MealTableSpread` のノード契約と420x190アセット寸法は維持し、MEAL_RESULTの左シーンだけ料理密度を上げる。
- 追記: 高密度化後の実スクショでは皿の右端が左シーン外へ切れやすかったため、MEAL_RESULT時の `MealSceneActor` をわずかに戻し、`MealTableSpread` の表示幅を収まる値へ下げる。人物を原寸より大きく保ったまま、焼き魚/レモン/椀がクリップしないことを実スクショで確認する。
- 追記: 上部の `MealResultBanner` は補足文を消し、`アジの塩焼きを食べた！` の1行見出しを主役にする。`meal_banner_frame.png` の左タブはEXP風の `+` 記号から食事用のフォーク/スプーン記号へ更新した。さらにMEAL_RESULTでは`RewardFlowRow`を非表示にし、常時の3ステップ導線はEXP_GAIN側へ残すことで、結果バナーを邪魔しない比重へ調整した。
- 追記: 最新スクショでは `MealResultBanner` の紙面に細い斜線が多く、主見出しの背面が少し素材テストのように見えた。参照02の食事完了バナーに寄せるため、`meal_banner_frame.png` は中央の文字面をクリーンな羊皮紙にし、紙端の影、薄い折り目、魚ウォーターマーク、濃紺の食事タブだけで状態感を出す。`MealResultSplitTitle` / `MealResultModeTab` のノード契約は変えず、文字可読性を優先する。
- 追記: 追加比較では上部バナーの斜線がまだ見出し背面のノイズとして目立ち、参照02の破れ羊皮紙より「線入りフレーム」に見えやすかった。`tools/generate_cooking_showcase_assets.py` の `meal_banner_frame()` を更新し、斜め折り目のアルファをさらに下げ、紙端の欠けと影を増やして素材感を端へ寄せる。中央の文字面は空けたまま、`MealResultSplitTitle` の可読性を優先する。
- 追記: 最新比較では上部バナーの右側魚スタンプが濃く、`アジの塩焼きを食べた！` の主見出しと競合していた。`meal_banner_frame()` の紙面装飾を別レイヤー合成へ変更し、魚スタンプや折り目が低アルファの穴として背面色を透かさないよう修正する。再生成した `meal_banner_frame.png` は魚を薄い印刷跡として残し、タイトル面は参照02に近い静かな羊皮紙へ寄せる。Godotの import cache は `--import` で更新し、`/tmp/tsuri_cooking_result.png` で新素材が反映されたことを確認する。
- 追記: 参照02の結果バナーは `アジの塩焼きを` と赤い `食べた！` が2段で読め、食事完了の山場が一目で分かる。現行の1行見出しは収まりは良いが、料理名と達成語が同じ強さで横に並び、参照02より少し弱かったため、MEAL_RESULT専用の `MealResultSplitTitle` を追加した。既存の `MealResultTitle` は監査用テキストとして保持し、描画では上段を濃い墨色、下段を赤で大きく描く。EXP_GAIN系では非表示にし、食事結果バナーだけの主見出しとして固定する。
- 追記: 参照02では上部バナーの左タブに `食べる` の状態名があり、食事結果の入口が小さく示されている。MEAL_RESULTでは左食事シーン内の大きな `食べる` 見出しは隠したまま、右上バナーのタブ内に `MealResultModeTab` として小さく `食べる` を戻す。結果バナーの主文言を邪魔せず、食事完了シーンであることを参照02に近い位置で補助する。EXP_GAIN系では非表示にし、状態名が混ざらないようcontent/layout auditで固定する。
- 追記: 実スクショでは `MealResultModeTab` の `食べる` が紙バナー上の小さな注釈に見えやすかったため、タブ右半分に濃紺のラベル面、金の区切り線、太めの白文字を描く。`meal_banner_frame.png` の左タブ形状とアイコンは維持しつつ、状態名が青タブ内の見出しとして読めるようにして、参照02の `食べる` タブへ近づける。EXP_GAIN系では引き続き非表示にし、食事結果だけの状態ラベルとして扱う。
- 追記: 参照02の上部バナーは食事完了の山場として星と暖色光が乗っている一方、現行スクショは紙バナーの文字だけに寄っていたため、MEAL_RESULT専用の `MealResultBannerSpark` を追加した。新しい説明文は増やさず、バナー背面に小さな金色スパーク、弧状ハイライト、端の発光を描いて `アジの塩焼きを食べた！` が画面上段の祝祭ピークとして読めるようにする。EXP_GAIN/EXP_GAIN_LEVELUPでは非表示にし、食事結果だけの演出としてcontent/layout auditで固定する。
- 追記: 最新スクショでは右上バナーの二段見出しは機能していたが、参照02の `食べた！` ほど画面上段の山場になり切っていなかった。MEAL_RESULT時だけ `MealResultBanner` の高さを少し増やし、その分 `MealDishCard` を詰めて右カラム全体の高さは維持する。`MealResultSplitTitle` は上段料理名と赤い `食べた！` を大きくし、赤い下線と薄い光の受けを足して、食事完了のピークが最初に読める状態へ寄せる。EXP_GAIN系ではこの見出しは引き続き非表示にする。
...
- 追記: 下段の4報酬カードとさらに下のステータス帯が同じ濃紺カード群として連続して見えやすかったため、MEAL_RESULT時だけ `_reward_grid` の背面に薄い共通台座を描く。新しい枠や文言は足さず、上辺/下辺の金線と弱い濃紺バンドで `基本EXP` / `初回ボーナス` / `合計獲得` / `次の釣行` をひとまとまりの報酬列として読ませ、ステータス帯は一段下の現在状態として分離する。
- 追記: 下段報酬カード列では `基本EXP` / `初回ボーナス` / `合計獲得` / `次の釣行` が均等に見えやすかったため、`RewardCardTotalExp` 内に `RewardTotalPeakGlow` を追加した。新しい文言やカードサイズ変更は入れず、合計EXPの背面だけに強めの金色レイ、楕円グロー、星粒を描き、参照02のように「今回の合計」が報酬カード列のピークとして最初に読める状態へ寄せる。EXP_GAIN系では報酬カード列ごと非表示になるため、content/layout auditではMEAL_RESULT表示、EXP_GAIN/EXP_GAIN_LEVELUP非表示を固定する。
...
- 追記: 参照02の `今回の料理` カードは、短い説明文ではなく料理写真と料理名そのものを見せる展示カードとして機能している。最新スクショでは補足文が右側の説明パネル感を残していたため、MEAL_RESULT時は `MealDishCard` の補足文を非表示にし、`RewardDishFeatureImage` の最小幅を広げる。`MealDishCardBridge` の料理写真側フレームも右へ広げ、分割線と銘板を控えめにして、料理写真 -> 料理名 -> 報酬カードの読みを維持する。content auditは補足文必須ではなく、料理カード、料理写真、料理名、報酬効果の存在を固定する契約へ更新する。
- 追記: `MealDishCard` 内は左の料理写真と右の料理名/短文が直線的に分かれて見えやすかったため、`MealDishCardBridge` をカード背景に追加した。料理写真側の暖色グロー、料理名側への薄い金ライン、小さな星粒を背面だけに描き、`今回の料理` カードが左右2分割の情報欄ではなく、料理写真から料理名、報酬カードへ流れる中継カードとして読めるようにする。EXP_GAIN系では非表示にし、MEAL_RESULTだけのカード内視線誘導としてcontent/layout auditで固定する。
- 追記: 参照02の `今回の料理` カードは右上の光と料理写真から文字側へ流れるハイライトがあり、食べた料理を報酬へつなぐ中継点として目に入る。現行ではカード右側が少し静かだったため、`MealDishCardBridge` に右上グリントと料理写真から料理名へ向かう暖色ラインを追加した。新しい文言は増やさず、料理名/短文の可読性を保ったまま、カードが報酬画面内の主役ブロックとして読めるようにする。
- 追記: 最新スクショでは `MealDishCard` の右側が濃紺の情報欄として平たく見え、参照02の料理名入り完成料理カードほど料理名が主役になっていなかった。MEAL_RESULT時だけ `MealDishCardBridge` に料理名用の暗い銘板、上辺ハイライト、写真側から銘板へ流れる暖色ラインを追加し、料理写真 -> 料理名 -> 報酬カードの中継点として読ませる。文言とカード寸法は変えず、EXP_GAINでは従来通り非表示に戻す。
- 追記: 最新比較では右の `MealDishCard` がまだ左右分割の説明パネルに見え、参照02の「料理写真を飾る展示カード」より素材の受けが弱かった。MEAL_RESULT時の `MealDishCardBridge` に料理写真側の展示台フレーム、金のコーナー、下辺影、料理名銘板の縦幅と薄い内側グローを追加する。文言やカード寸法は変えず、料理写真と料理名を1つの完成料理カードとして読ませる。
- 追記: 右の `MealDishCard` で料理写真の表示領域だけ広げると、実スクショでは写真右側に未使用のグレー帯が残り、展示カードではなく素材枠の空きに見えた。MEAL_RESULT時だけ `RewardDishFeatureImage` を `STRETCH_KEEP_ASPECT_COVERED` に切り替え、焼きアジの皿が料理写真枠を埋めるようにする。EXP_GAINでは `STRETCH_KEEP_ASPECT_CENTERED` に戻し、左カード内の料理ソース表示が過剰にクロップされない状態分離を維持する。
...
- 追記: 参照02の `効果を得た` カードはEXPカードとは別に、内側の緑枠と大きな効果メダルで「次回の釣行へ持ち越す報酬」として読める。現行の `RewardCardNextEffect` は数値カード列の4枚目に見えやすかったため、`RewardBuffEffectPlate` を追加し、緑の内枠、左メダル、右側の効果レーンを背面に描く。新しい説明文は足さず、`RewardBuffSignal` と `最大体力 +5% / 次回1回で発動` の可読性を保ったまま、効果カードをEXPカードとは異なる報酬種別として固定する。
...
- 追記: 実スクショでは `MealResultRewardCue` のラインが右側だけに寄り、下段4報酬カード全体をまとめる力が弱かったため、報酬列の上辺全体に薄い横光を追加し、4カード中心へ落ちる小さな下向きシェブロンを描く。新しい文言や操作要素は足さず、`MealDishCard` から `基本EXP` / `初回ボーナス` / `合計獲得` / `次の釣行` へ視線が落ちる導線として扱う。EXP_GAIN系では引き続き非表示にし、MEAL_RESULTだけの報酬列グルーピングとして固定する。
...
- 追記: 参照02の報酬カードは背景演出が強くても数値の背面に暗い受けがあり、`+60 EXP` / `+90 EXP` が先に読める。現行ではカード内の放射線と薄絵が値の周りまで走り、下部ステータス帯と同じ密度に見えやすかったため、`RewardValuePlateVisual` を追加し、各報酬値の背後に暗いプレートと細いアクセント線を描く。MEAL_RESULTではカード高さと主要値フォントも上げ、`基本EXP` / `初回ボーナス` / `合計獲得` / `次の釣行` が説明文ではなく獲得値として読める状態をlayout auditで固定する。
- 追記: 最新スクショでは報酬値は読めるが、カード上部の `基本EXP` / `初回ボーナス` / `合計獲得` / `次の釣行` が背景面に直接乗り、参照02の「見出し付き獲得カード」よりUI素材感が弱かった。`_draw_reward_header_ribbon` を追加し、各報酬カードの見出し背面に濃紺の銘板、金線、種別アクセント、合計獲得だけの弱いグローを描く。文言・カードサイズ・報酬値は変えず、MEAL_RESULTの4カードが下部ステータス帯ではなく報酬列として先に読める状態へ寄せる。
...
- 追記: 実スクショでは `RewardConfirmButton` のサイズは確保できたが、報酬カード列からEXP加算画面へ進む終点としてはまだ控えめだったため、MEAL_RESULT時だけボタン面に薄い金/シアンの導線レール、右向きシェブロン、右端のEXP玉を描く。既存の椀からEXP玉へ進む左側キューは維持し、EXP_GAIN/LEVELUP側には入れないことで、画面2だけの「食べた報酬を食経験値へ送る」締めとして扱う。
- 追記: 最新スクショでは報酬カード列の整理が進んだ分、最後の `食経験値へ進む` ボタンがまだ細い下端操作に見えやすかった。MEAL_RESULT時だけ `RewardConfirmButton` 内の暗い受け、金/シアンのレール、下向きの小シェブロンを強め、報酬列からEXP加算へ進む着地点として読ませる。ボタンサイズと文言は変えず、EXP_GAIN/LEVELUP側の導線は従来強度のままにする。
...
- 追記: 参照02の下部ステータス帯は補助情報だが、値とアイコンは十分大きく、食事結果が現在準備に反映されたことを支えている。現行では `Lv.`、クーラー、所持金、EXP小数値が下段の装飾線に埋もれやすかったため、`RewardStatusStrip` と各カードの高さ、アイコン幅、主要値フォント、EXPバー/数値幅を少し引き上げた。MEAL_RESULTでは報酬カードとCTAを主役に保ちつつ、ステータス帯が読める現在状態として残ることをlayout auditで固定する。
- 追記: 最新スクショでは下部ステータス帯の値は読めるが、個別カード内の星粒/光が報酬カードと同じ密度に見え、`基本EXP` から `食経験値へ進む` までの主導線を少し濁していた。MEAL_RESULT時だけ `RewardStatusStrip` 背面に薄い補助HUD用シェルフを敷き、各 `RewardStatus*Card` の装飾グロー/小スパークを抑える。サイズ、値、アイコンは維持し、EXP_GAINでは従来通り強いステータス確認カードへ戻す。
- 追記: 参照02と最新 `/tmp/tsuri_cooking_result.png` を比べると、左食事シーンと上部バナーは成立している一方、下段の4報酬カードがまだ低く、下部ステータス帯と同じ計器列に見えやすかった。MEAL_RESULT時だけ `RewardCard*` の高さを142pxへ上げ、`基本EXP` / `初回ボーナス` / `合計獲得` の数値を拡大し、カード内に `_draw_reward_award_well` の暗い受け皿を追加する。本文説明は戻さず、見出し、値、薄絵、受け皿で報酬カードを主役化する。相殺として `MealDishCard` を少し詰め、MEAL_RESULT時の `RewardStatusStrip` と `RewardStatus*Card` は最小高を下げる。EXP_GAIN側のコンパクト表示は維持する。
- 凍結判定: 2026-06-30時点の最新 `/tmp/tsuri_cooking_result.png` と `reference/cooking_flow/02_meal_result_concept.png` の比較では、画面2にP1は残っていない。`食べた！` バナー、今回の料理、基本EXP/初回/合計/次回効果、`食経験値へ進む`、食後ステータスが本文を読まなくても追えるため、MEAL_RESULTは「食事結果の報酬シーン」として次画面へ渡せる。
- 領域別再判定: 左食事シーンはP2残りだが、人物・料理・窓辺・湯気が一枚絵として読め、破綻ではないため凍結候補。上部バナーはP3で、二段見出しと赤い `食べた！` が十分に主役化しているため凍結。料理カードはP3で、料理写真の空き帯も解消済みのため凍結。報酬カードはP2軽微で、合計獲得の圧倒感は参照02より弱いが、4報酬の分離と主要値の可読性は成立しているため凍結候補。CTA/ステータス帯はP2軽微からP3で、導線と現在状態は読めるため凍結候補。
- 次判断: 画面2の追加作業は、上部バナー紙面、左食卓の接続光、報酬カードのさらなる主役化など、いずれもP3寄りの微調整に入る。水中ファイトで有効だった凍結ルールに従い、MEAL_RESULTはここで凍結し、次は `EXP_GAIN` 画面3の主役構図・EXPメーター・料理からEXPへの導線を参照03に対して評価/改善する。MEAL_RESULTを再開する条件は、後続画面との横並び比較で明確なP1退行、文字クリップ、報酬導線の破綻、またはアセット差し替え時の大きな見た目崩れが出た場合に限る。
...
- 残る差分: 報酬カード列は下部ステータス帯より前に出たが、参照02のような高密度の人物・料理・窓辺まで一体化した一枚絵にはまだ届かない。次パスで触るなら、左背景と人物の接続光、または上部バナーの紙/魚スタンプの本番素材感を比較対象にする。
