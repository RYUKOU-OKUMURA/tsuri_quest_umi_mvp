...
3. **調理場R5/R1継続**: 現フェーズはCOOK_SELECTを参照画像へ近づけるreference-uplift。料理カード、下部バー、右詳細パネルは完了。次は背景の見せ方を、freeze値と衝突しない範囲で厨房背景が左右パネル間や外周に見える構成へ寄せる。同じ調理場スライスで触った行のPalette移行を併走し、`src/ui/cooking_screen.gd` の残hexは一括置換せず、触った状態・部品ごとに移行する。
