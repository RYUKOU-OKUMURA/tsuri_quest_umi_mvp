...
3. **調理場R5/R1継続**: 現フェーズはCOOK_SELECTを参照画像へ近づけるreference-uplift。料理カードは完了。次は下部バーを、参照の「プレイヤーLv / 効果中の料理 / クーラーボックス / 所持金」4区画へ寄せる。同じ調理場スライスで触った行のPalette移行を併走し、`src/ui/cooking_screen.gd` の残hexは一括置換せず、触った状態・部品ごとに移行する。
