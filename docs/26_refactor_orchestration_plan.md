...
3. **調理場R5/R1継続**: 現フェーズはCOOK_SELECTを参照画像へ近づけるreference-uplift。料理カードと下部バーは完了。次は右詳細パネルの「次の釣行で得られる効果」行、必要素材行、獲得EXP行を参照のバッジ/帯/アイコン質感へ寄せる。同じ調理場スライスで触った行のPalette移行を併走し、`src/ui/cooking_screen.gd` の残hexは一括置換せず、触った状態・部品ごとに移行する。
