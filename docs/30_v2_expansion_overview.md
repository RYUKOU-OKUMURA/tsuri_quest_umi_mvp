...
| Release Gate 1 / SAVE-01 | 完了 | 2026-07-11 | 将来版を含むslotをmain / backup / tmp単位で非破壊guardし、タイトルで対応版を案内。`version`欠損の旧疎saveは互換維持、数値以外の`version`は未知形式としてguardする。他slotの継続利用・slot切替再評価をsave smokeで確認 |
| E7 | 未着手 | — | ローンチ前。E5 の後 |
