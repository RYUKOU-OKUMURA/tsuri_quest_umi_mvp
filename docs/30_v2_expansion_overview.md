...
作成日: 2026-07-06
状態: 仕様確定・V2実装進行中（E0〜E4・E6・E10・docs/36・docs/38・docs/39・docs/35 P1 完了済み。現在 docs/35 P2/P3 継続中。全体ロードマップは「現在地」を参照）
...
1. **docs/35 P2/P3（魚素材の判別困難17種＋境界再評価）** — 本線と並走可の素材トラック（`tools/` + `assets/showcase/fish/` のみ。本線とファイルが被らないことを brief 側で保証する）。P1は完了済み。P2バッチ1で現監査pendingは0になったが、`template` 由来のP2残8種とP3再評価は継続
...
| E5 | 未着手 | — | ローンチ前。docs/36・docs/35 P1 は完了済み。docs/35 P2/P3 はE8/E9前ゲートとして並走継続 |
| E7 | 未着手 | — | ローンチ前。E5 の後 |
...
| docs/35 P2（B群17種）・P3 | 進行中 | P1 の後。**E8/E9 前に完了必須** | P2バッチ1で9種差し替え、監査pending 8→0 / strict通過。残P2 source化8種 + P3再評価 |
