...
作成日: 2026-07-06
状態: 仕様確定・V2実装進行中（E0〜E4・E6・E10・docs/36 完了済み。次は docs/38（餌魚READYセレクタ＋耐久チャージ）→ docs/39（水中ファイト刷新）→ E5。全体ロードマップは「現在地」を参照）
...
E0〜E4・E6・E10 は実装・検証・台帳更新まで完了済み。以後の作業者は、これらを前提実装として扱う。
...
残り作業のロードマップ（2026-07-07確定。品質トラック docs/33・35・36 を含む全体順）:

1. **docs/38（餌魚READYセレクタ＋レア度耐久チャージ）** — docs/36 実装後のUX再検討（docs/37）で E10-4 の消費モデルを改訂。E4+E10 同時リリース（決定#14）の実質的な残作業。最優先
2. **docs/39（水中ファイト画面 基盤UI刷新）** — 決定#19（基盤レイアウト原則）の適用。docs/38 の直後に続けて釣行画面ファミリーの回帰を1回で済ませる
3. **docs/35 P1（魚素材の種描写誤り15種）** — 本線と並走可の素材トラック（`tools/` + `assets/showcase/fish/` のみ。本線とファイルが被らないことを brief 側で保証する）。再発防止監査の validate 組み込みまで含め、**E8/E9 着手前に必ず完了**させる。P2（判別困難17種）は続けてバッチで流す
4. **E5 → E7** — 本線再開。E7 は「難易度=新規セーブ時のみ」（決定#3）のためローンチ前必須。E5 もセーブスキーマ・港UIに触るためローンチ前に枯らす
5. **E11 ＋ docs/33 の M0/C0（P1掃除のみ）** — ゲーム内容が固まってから蓋を閉める。§3-4 の未決3件（期限到来済み）のユーザー決定が着手前提
6. **=== ローンチ ===**（スコープは決定#18）
7. **docs/33 素材供給ライン確立（M1〜M3 → C1〜C5）→ E8 → E9** — ローンチ後。E9 は横展開の仮説検証のため、ローンチ反応を見てから着手判断
...
**⚠ 期限到来（2026-07-07）**: 下記1・3・4 は決定期限が「E10完了まで」で、E10 は完了済み。**いずれも E11 の作業内容そのものを左右するため、E11 着手前にユーザー決定が必須**。docs/36 を進めている間に確定させる。
...
| E5 | 未着手 | — | ローンチ前。docs/36・35P1 の後 |
| E7 | 未着手 | — | ローンチ前。E5 の後 |
| E11 | 一部着手 | — | docs/31 素材台帳を作成済み。実装部は E7 後。§3-4 の未決3件（期限到来）が着手前提 |
| E8 | 未着手 | — | **ローンチ後**（決定#18）。docs/35 の再発防止監査 validate 組み込み後に着手 |
...
| docs/38 餌魚READYセレクタ＋耐久チャージ | 仕様確定・未着手 | **最優先**（docs/36の消費タイミング不変前提を上書き） | READYで餌魚選択・キャスト時消費・レア度チャージ・危険海域のみ抽選遅延。docs/37が経緯、docs/38が実装正本 |
| docs/39 水中ファイト基盤UI刷新 | 仕様確定・未着手 | docs/38 の直後（決定#19の適用） | 下段スリムバー・サイドバー廃止・フローティングカード。読み順は docs/39 §0 |
| docs/35 魚素材重複 P1（A群15種） | 未着手 | E5/E7 と並走可（`tools/`・`fish/` のみ） | 種描写誤りの修正。再発防止監査を validate へ組み込み |
| docs/35 P2（B群17種）・P3 | 未着手 | P1 の後。**E8/E9 前に完了必須** | 判別困難の解消 |
...
- 2026-07-07: READY UI理想イメージ `reference/13` の生成を経て、ユーザー決定「この様式をゲームプレイ画面の基盤UIデザインとする」（決定#19）。`reference/14`（FIGHT）を追加生成し、原則を `docs/19` §4.6 に明文化、水中ファイト刷新仕様を `docs/39` に確定。docs/38 §4 もREADY専用下段バーへ改訂。実装順 = docs/38 → docs/39 → E5。以後の実装者（Codex）は docs/39 §0 のドキュメントマップから読み始める
