...
flowchart LR
  ...
  RIGHTSB --> RCV
  TARGET["最低対象Mac・Intel確認"] --> PRE
  SIGN["署名・公証方針（確定）"] --> PRE
  DOCS["製品文書"] --> PRE
  VERIFY["release verifier骨格"] --> PRE
~~~

`RC固定` は検証対象のsource commit・Godot / export template版・成果物hashを不変にする境界とする。素材差し替えへ発展し得るRIGHTS-01A、最低対象Mac・Intel確認、署名/公証方針、README等のsource-controlled製品文書、release verifier骨格はPre-RCで閉じる。SIGN-01の方針は2026-07-18に確定済みだが、Apple Developer Program加入、証明書発行、署名・公証・staple実行証拠は外部pendingであり、RC生成までに満たす。RC生成時にunsigned export hashを記録し、同じsourceから署名・公証済み配布物を作ってfinal artifact / ZIP hashを別々に記録する。以後のclean起動・性能・9セル受入は**最終配布物そのもの**を対象にする。修正・再export・再署名でfinal hashが変わった場合はRC番号を上げ、影響検証に加えてrelease verifier全体を再実行する。検証ログは固定したsource commit / final artifact hashを参照する証拠として後続commitへ保存できるが、パッケージ入力を変えない。
...
| D: external decision / acceptance | TARGET-01の最低macOS・最低対象Mac・Intel確認、SIGN-01のApple加入・証明書・署名/公証証拠、itch.io設定 | ユーザー/外部サービスでしか確定できない証拠と判断。SIGN-01の発売方針自体は確定済み | PERF、最終配布 |
...
| Wave 1: 即時3並列 | A=`SAVE-03`、B=`最小export spike`、C=`RIGHTS-01A`の既存台帳とU-01〜U-08対応表。DはTARGET-01の最低対象Mac・Intel確認を継続。SIGN-01の署名/公証方針は2026-07-18確定済みで、加入・証明書・実行証拠を継続 | 各レーンを独立commit。共有ファイル競合0 |
...
| TARGET-01 | ユーザー判断待ち | 最低macOS、最低対象Mac、Intel確認方法を確定 |
| SIGN-01 | **方針確定・外部実行待ち** | 初回itch.io版RCは`Developer ID Application`署名＋Hardened Runtime＋`notarytool`公証＋署名済み`.app`へのstapleを必須とする。Apple Developer Program未加入、証明書未発行、署名・公証・staple実行証拠は外部pending |
...
| docs/45 リリース前監査フォローアップ | 是正中 | **現在の最優先** | Release Gate 0/1・ID-01・REL-01最小export・SAVE-01〜04・UI-QUEST-01・UI-READY-01・M0・C0・QA-RELEASE骨格・DOCS-RELEASE草案・E7は完了。E11、RIGHTS、TARGET、SIGN外部実行、固定RC検証が残る。詳細briefはdocs/45 |
