...
作成日: 2026-07-06
状態: 仕様確定・V2実装進行中（E0〜E6・E10、docs/35〜44の完了分、Release Gate 0/1、ID-01、REL-01最小export、SAVE-01〜04・UI-QUEST-01・UI-READY-01・docs/33 M0/C0、QA-RELEASE骨格、DOCS-RELEASE草案を前提化。E7、E11、RIGHTS-01A、TARGET-01 / SIGN-01、固定RC検証は未完）
...
| E7 | **着手可** | Release Gate 1完了。Wave 3のcore / fight / UI境界で実装 |
...
| QA-RELEASE | **骨格完了** | scene 28件＋script-only audit 1件をmanifest完全一致で列挙。run固有HOME / logs、source・manifest開始終了hash、未知ERROR / WARNING拒否、RC成果物再hashを実装し、skeleton 29件をclean treeで通過。exportは`pending_rc_evidence`で、固定RCの`--rc`実行は未完 |
...
| QA-RELEASE（release verifier骨格） | 完了 | 2026-07-11 | 29対象の自動列挙＋manifest完全一致、特殊3件presence、process-group timeout、run固有HOME / logs、source・manifest安定性、未知WARNING拒否、RC証跡開始終了再hashを実装。clean skeletonは29件PASS、固定RC evidenceは未入力 |
...
| E7 | 着手可 | — | ローンチ前。Release Gate 1完了済み。Wave 3でcoreを先に統合し、fight / UIを並列実装 |
