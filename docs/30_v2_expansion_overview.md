...
- `save_game()`は書き出すruntime dataにも同じ意味検証をtmp作成前に適用し、範囲外ならmain / backup / tmpを変更せず失敗通知する。通常操作による所持金・EXP・捕獲/所持/料理/依頼回数の加算と売却額・売却数の積算はJSON安全整数上限でsaturateする。売値×数量はint64積を作る前に上限判定し、正常saveが次回loadで自己破損しない
