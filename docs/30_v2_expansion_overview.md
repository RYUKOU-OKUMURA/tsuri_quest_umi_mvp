...
4. **E11（進行中）** — SETTINGS-AUDIO、SLOT-DELETE UI、DISPLAYは完了。残りはINPUT-COMMON、EXTERIOR、最終受入。docs/33 M0/C0と依頼/READYの局所P1は完了済み
...
| E11実装 | **進行中** | SETTINGS-AUDIO、セーブ枠削除backend/UI、DISPLAY（fullscreen保存・起動時適用・keep＋黒帯の実runtime matrix QA）を完了。settings spineはINPUT-COMMON → EXTERIOR → 最終受入の順で継続 |
...
| E11 | 進行中 | — | SETTINGS-AUDIO、セーブ枠削除backend/UI、DISPLAY（fullscreen保存・起動時適用・keep＋黒帯の実runtime matrix QA）、入力・解像度QA harnessを完了。残りはINPUT-COMMON、EXTERIORと最終受入 |
