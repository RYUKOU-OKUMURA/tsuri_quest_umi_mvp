...
4. **E11（進行中）** — SETTINGS-AUDIO、SLOT-DELETE UI、DISPLAY、INPUT-COMMONは完了。残りは画面別INPUT修正、EXTERIOR、最終受入。docs/33 M0/C0と依頼/READYの局所P1は完了済み
...
| E11実装 | **進行中** | SETTINGS-AUDIO、セーブ枠削除backend/UI、DISPLAY、INPUT-COMMONを完了。実キー8 action、共通focus / cancel契約、13画面registryと3段階harnessを統合し、最新baselineは42件（P1 34 / P2 8）。次は画面別INPUTを最大3並列で修正し、owner境界とU-04解消後のEXTERIOR、最終受入へ進む |
| QA-RELEASE | **骨格完了** | settings smoke追加後のscene 30件＋script-only audit 1件、合計31対象をmanifest完全一致で列挙。run固有HOME / logs、source・manifest開始終了hash、未知ERROR / WARNING拒否、RC成果物再hashを実装し、固定RCの`--rc`実行は未完 |
...
| QA-RELEASE（release verifier骨格） | 完了 | 2026-07-11 | settings smoke追加後の31対象を自動列挙＋manifest完全一致で管理。セーブ2件・settings smoke 1件・export 1件の特殊runner 4件presence、process-group timeout、run固有HOME / logs、source・manifest安定性、未知WARNING拒否、RC証跡開始終了再hashを実装。固定RC evidenceは未入力 |
...
| E11 | 進行中 | — | SETTINGS-AUDIO、セーブ枠削除backend/UI、DISPLAY（fullscreen保存・起動時適用・keep＋黒帯の実runtime matrix QA）、INPUT-COMMONを完了。残りは42件の画面別INPUT修正、EXTERIORと最終受入 |
...
| docs/54 UIブラッシュアップ総合計画 | 現行 | I0完了、次は画面別INPUT | 共通基盤を統合し、INPUT-TITLE / FISHING / SHIPYARDを先頭とする画面別3並列→C1-B/R5-B/FIGHT authored 3並列を定義 |
