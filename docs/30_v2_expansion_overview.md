...
作成日: 2026-07-06
状態: 仕様確定・V2実装進行中（E0〜E6・E10、docs/35〜44の完了分、Release Gate 0、ID-01、SAVE-01〜03・UI-QUEST-01・UI-READY-01・docs/33 M0/C0を前提化。最小export spike、SAVE-04、E7、E11、release_verifyは未完）
...
1. **Release Gate 0・ID-01（2026-07-11完了）→ 最小export spike** — user data namespace、macOS bundle ID、itch.io page slug / store App ID、旧MVP saveのコピー移行境界まで確定済み。後続spikeでbundle IDをpresetへ配線し、debug / release exportとclean起動・save再読込を確認
2. **Release Gate 1残（SAVE-04）** — SAVE-01の将来版セーブ保護、SAVE-02の保存失敗伝播、SAVE-03の意味検証付きbackup選択は完了。旧依頼repairを次に実施。詳細とbriefはdocs/45
...
| `inventory`, `caught_counts` | 任意 | Dictionary。各valueは数学的な整数、0以上かつJSON安全整数上限以下 |
...
| `eaten_recipes` | 任意 | Dictionary（valueは既存互換のため制約しない） |
...
| `quest_board` | 任意 | Array。各要素はDictionary。既知fieldがあればID / kindはString、count / rewardは0以上かつJSON安全整数上限以下の整数、sizeは有限かつ0以上。魚IDの存在・達成可否repairはSAVE-04の責務 |
...
- 意味破損fixture: `{}`、`{"unknown_only": true}`、`{"version": 1, "level": {}}`、`{"version": 1, "money": -1}`、`{"version": 1, "money": 0.5}`、`{"version": 1, "money": 1e100}`、`{"version": 1, "play_seconds": null}`、`{"version": 1, "inventory": []}`、`{"version": 1, "spot_caught_counts": {"harbor_pier": []}}`、`{"version": 1, "spot_caught_counts": {"harbor_pier": {"aji": -1}}}`、`{"version": 1, "quest_board": ["broken"]}`
...
| SAVE-04 | **着手可** | 未知魚・サメ・ヌシ依頼の除去、正常依頼維持、3件補充 |
...
| SAVE-03（Release Gate 1） | 完了 | 2026-07-11 | version 1の必須なし・既知key型/範囲契約を固定し、main / backupへ同じ意味検証を適用。loadとtitle要約の候補を一致させ、意味破損mainから正常backupへのfallback、両候補不正時の通知・原本hash維持、旧疎save互換をsave smokeで確認。Release Gate 1全体はSAVE-04を継続 |
...
| E7 | 未着手 | — | ローンチ前。Release Gate 1（SAVE-04残）完了後、Wave 3で実装 |
...
| docs/45 リリース前監査フォローアップ | 是正中 | **現在の最優先** | Release Gate 0・ID-01・SAVE-01〜03・UI-QUEST-01・UI-READY-01・M0・C0は完了。最小export spike、SAVE-04、E7、E11、release_verifyが残る。詳細briefはdocs/45 |
