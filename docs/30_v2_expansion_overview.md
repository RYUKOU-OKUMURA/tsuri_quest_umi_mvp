...
作成日: 2026-07-06
状態: 仕様確定・V2実装進行中（E0〜E6・E10・docs/36・docs/38・docs/39・docs/35 完了済み。全体ロードマップは「現在地」を参照）
...
1. **E7** — 本線の次フェーズ。E7 は「難易度=新規セーブ時のみ」（決定#3）のためローンチ前必須
...
| E5 | 完了 | 2026-07-08 | `selected_time_slot_id`、時間帯別出現重み、港セレクタ、港/釣行グレーディング、夜BGM override、監査/QAを追加 |
| E7 | 未着手 | — | ローンチ前。E5 の後 |
