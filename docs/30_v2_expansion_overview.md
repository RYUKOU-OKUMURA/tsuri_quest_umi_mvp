...
作成日: 2026-07-06
状態: 仕様確定・V2実装進行中（E0〜E4・E6・E10・docs/36・docs/38・docs/39・docs/35 P1/P2 完了済み。現在 docs/35 P3 継続中。全体ロードマップは「現在地」を参照）
...
1. **docs/35 P3（魚素材の境界再評価）** — 本線と並走可の素材トラック（`tools/` + `assets/showcase/fish/` のみ。本線とファイルが被らないことを brief 側で保証する）。P1/P2は完了済み。`megochi`, `kurosoi/takenokomebaru`, `mejina` の再クラスタリングと据え置き/修正判定を残す
2. **E5 → E7** — 本線再開。E5 はセーブスキーマ・港UIに触るため、釣行画面の基盤UI回帰を閉じた状態から着手する。E7 は「難易度=新規セーブ時のみ」（決定#3）のためローンチ前必須
3. **E11 ＋ docs/33 の M0/C0（P1掃除のみ）** — ゲーム内容が固まってから蓋を閉める。§3-4 の未決3件（期限到来済み）のユーザー決定が着手前提
4. **=== ローンチ ===**（スコープは決定#18）
5. **docs/33 素材供給ライン確立（M1〜M3 → C1〜C5）→ E8 → E9** — ローンチ後。E9 は横展開の仮説検証のため、ローンチ反応を見てから着手判断
...
| E5 | 未着手 | — | ローンチ前。docs/36・docs/35 P1/P2 は完了済み。docs/35 P3 はE8/E9前ゲートとして並走継続 |
| E7 | 未着手 | — | ローンチ前。E5 の後 |
...
| docs/35 P2（B群17種）・P3 | 進行中 | P1 の後。**E8/E9 前に完了必須** | P2バッチ1+2で17種差し替え済み、監査pending 0 / strict通過。残P3再評価 |
...
- 2026-07-08: docs/35 P2バッチ2として残8種（`shimaaji`, `gingameaji`, `kaiwari`, `ishigarei`, `umitanago`, `ishigakidai`, `oomonhata`, `ara`）を新規コンタクトシート由来へ差し替え、P2 B群17種を完了。監査pending 0 / strict通過を維持。残作業はP3再評価
