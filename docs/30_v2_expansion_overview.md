...
作成日: 2026-07-06
状態: 仕様確定・V2実装進行中（E0〜E4・E6・E10 完了済み。次は docs/36 フォローアップ → E5。全体ロードマップは「現在地」を参照）
...
1. **docs/36（サメ餌魚UX）フェーズ1→2** — E4+E10 同時リリース（決定#14）の実質的な残作業。最優先
2. **docs/35 P1（魚素材の種描写誤り15種）** — 本線と並走可の素材トラック（`tools/` + `assets/showcase/fish/` のみ。本線とファイルが被らないことを brief 側で保証する）。再発防止監査の validate 組み込みまで含め、**E8/E9 着手前に必ず完了**させる。P2（判別困難17種）は続けてバッチで流す
3. **E5 → E7** — 本線再開。E7 は「難易度=新規セーブ時のみ」（決定#3）のためローンチ前必須。E5 もセーブスキーマ・港UIに触るためローンチ前に枯らす
4. **E11 ＋ docs/33 の M0/C0（P1掃除のみ）** — ゲーム内容が固まってから蓋を閉める。§3-4 の未決3件（期限到来済み）のユーザー決定が着手前提
5. **=== ローンチ ===**（スコープは決定#18）
6. **docs/33 素材供給ライン確立（M1〜M3 → C1〜C5）→ E8 → E9** — ローンチ後。E9 は横展開の仮説検証のため、ローンチ反応を見てから着手判断
...
| トラック | 状態 | 位置づけ | 備考 |
|---|---|---|---|
| docs/36 サメ餌魚UX | 未着手 | **最優先**（E4+E10 同時リリースの残作業） | フェーズ1→2。抽選・消費仕様は不変 |
