...
作成日: 2026-07-06
状態: 仕様確定・V2実装進行中（E0〜E4・E6・E10・docs/36・docs/38・docs/39 完了済み。次は docs/35 P1 → E5。全体ロードマップは「現在地」を参照）
...
1. **docs/35 P1（魚素材の種描写誤り15種）** — 本線と並走可の素材トラック（`tools/` + `assets/showcase/fish/` のみ。本線とファイルが被らないことを brief 側で保証する）。再発防止監査の validate 組み込みまで含め、**E8/E9 着手前に必ず完了**させる。P2（判別困難17種）は続けてバッチで流す
...
| docs/35 魚素材重複 P1（A群15種） | 完了 | E5/E7 と並走可（`tools/`・`fish/` のみ） | 再発防止監査は validate 組み込み済み。P1バッチ1+2で15種差し替え、監査pending 26→8 |
| docs/35 P2（B群17種）・P3 | 未着手 | P1 の後。**E8/E9 前に完了必須** | 判別困難の解消。残pending 8件 |
