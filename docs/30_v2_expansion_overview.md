...
作成日: 2026-07-06
状態: 仕様確定・V2実装進行中（E0〜E6・E10、docs/35〜44の完了分、Release Gate 0、SAVE-01・UI-QUEST-01・UI-READY-01・docs/33 M0/C0を前提化。SAVE-02〜04、E7、E11、release_verifyは未完）
...
1. **Release Gate 0（2026-07-11完了）→ ID-01** — 販売先・対象OS・入力・非16:9・正式名称は決定#20で確定済み。後続ID-01でuser data namespace / OS application ID / store App IDを分離し、権利証跡、最小export spikeを先行
...
後続ID-01では、Godotのuser data namespace、macOSのOS application / bundle ID、itch.ioのstore App IDを別々の値として記録する。store App IDが未発行なら「未発行」と記録する。正式名称のruntime反映はuser data namespaceと旧save移行方針の固定後に行う。
...
| docs/45 リリース前監査フォローアップ | 是正中 | **現在の最優先** | Release Gate 0・SAVE-01・UI-QUEST-01・UI-READY-01・M0・C0は完了。SAVE-02〜04、E7、E11、release_verifyが残る。詳細briefはdocs/45 |
