...
作成日: 2026-07-06
状態: 仕様確定・V2実装進行中（E0〜E6・E10、docs/35〜44の完了分、SAVE-01・UI-QUEST-01・UI-READY-01・docs/33 M0/C0を前提化。Release Gate 0、SAVE-02〜04、E7、E11、release_verifyは未完）
...
| docs/33 M0（市場P1掃除） | 完了 | E11 と同梱（ローンチ品質） | 空状態の氷トレー・行残骸・未ロード風バーをruntime空状態表示へ置換。2026-07-11に被覆残像を再修正し、4状態QA・包含smoke・validateを実施 |
| docs/33 C0（調理P1掃除） | 完了 | E11 と同梱（ローンチ品質） | 残像wash・タイトル帯衝突・導線グリフ・金額formatを修正。構造契約はheadless `cooking_verify.sh`、実画像は独立 `cooking_visual_qa.sh` で検証 |
| docs/45 リリース前監査フォローアップ | 是正中 | **現在の最優先** | SAVE-01・UI-QUEST-01・UI-READY-01・M0・C0は完了。Release Gate 0、SAVE-02〜04、E7、E11、release_verifyが残る。詳細briefはdocs/45 |
...
- 2026-07-10: docs/45 UI-READY-01を完了。READY下段バー全体は動かさず、最長餌魚名の2行固定スロットと3桁在庫・残チャージの状態別visual QAを追加。旧末尾省略案を不採用としてfreeze
- 2026-07-10: 全体コード・UI・発売外周監査をdocs/45へ保存。Release Gate 0/1、セーブ保護、局所UI P1、release_verifyをE7/E11前後の実行キューへ追加
- 2026-07-10: docs/45 UI-QUEST-01を局所修正。依頼札の主条件本文を肖像下の全幅3行領域へ移し、現行最長魚名 `タケノコメバル` を含む動的5テンプレートの全文表示を smoke / visual QA で固定。3列・外枠・依頼ロジックは不変。
- 2026-07-10: docs/33 M0を完了。市場の空状態を意図済み表示へ切替え、氷トレー・行UI残骸・未ロード風バーをP1として解消。4状態visual QA、空→魚あり→空のmarket smoke、validateを記録（証拠・freeze再オープン履歴は `docs/qa/fish_market_qa.md`）。
- 2026-07-11: docs/33 M0のレビュー差し戻しを修正。左行群と右詳細/価格/装飾の被覆矩形を実領域より外へ広げ、空状態の通常詳細を非表示化。4状態visual QA、実矩形の横・縦包含と空→再入荷→空を検証するmarket smoke、validateを再実施。
- 2026-07-10: docs/33 C0を完了。MEAL_RESULT残像wash、STATUS_SUMMARYタイトル帯衝突、EXP_GAIN / LEVEL_UP下部導線グリフ潰れ、STATUS_SUMMARY金額の3桁区切り違反をruntimeと表示契約で修正。C1〜C5は未着手のまま維持
- 2026-07-11: docs/33 C0のレビュー是正を反映。`cooking_verify.sh` からGUI/SubViewport必須のvisual QAを外し、Reward `88px`・LevelUp `92px`・命名済み単一cueをheadless content auditで個別に固定。実画像比較は独立した `cooking_visual_qa.sh` を維持
