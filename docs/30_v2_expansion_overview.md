...
作成日: 2026-07-06
状態: 仕様確定・V2実装進行中（E0〜E4・E6・E10・docs/36・docs/38 完了済み。次は docs/39（水中ファイト刷新）→ E5。全体ロードマップは「現在地」を参照）
...
1. **docs/39（水中ファイト画面 基盤UI刷新）** — 決定#19（基盤レイアウト原則）の適用。docs/38 のREADY刷新後に続けて釣行画面ファミリーの回帰を1回で済ませる
...
3. **E5 → E7** — 本線再開。E5 はセーブスキーマ・港UIに触るため、釣行画面の基盤UI回帰を先に閉じてから着手する。E7 は「難易度=新規セーブ時のみ」（決定#3）のためローンチ前必須
