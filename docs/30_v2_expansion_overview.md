...
作成日: 2026-07-06
状態: 仕様確定・V2実装進行中（E0〜E4・E6・E10・docs/36・docs/38・docs/39・docs/35 完了済み。全体ロードマップは「現在地」を参照）
...
1. **E5 → E7** — 本線再開。E5 はセーブスキーマ・港UIに触るため、釣行画面の基盤UI回帰とdocs/35魚素材ゲートを閉じた状態から着手する。E7 は「難易度=新規セーブ時のみ」（決定#3）のためローンチ前必須
2. **E11 ＋ docs/33 の M0/C0（P1掃除のみ）** — ゲーム内容が固まってから蓋を閉める。§3-4 の未決3件（期限到来済み）のユーザー決定が着手前提
3. **=== ローンチ ===**（スコープは決定#18）
4. **docs/33 素材供給ライン確立（M1〜M3 → C1〜C5）→ E8 → E9** — ローンチ後。E9 は横展開の仮説検証のため、ローンチ反応を見てから着手判断
...
| E5 | 未着手 | — | ローンチ前。docs/36・docs/35 は完了済み |
| E7 | 未着手 | — | ローンチ前。E5 の後 |
