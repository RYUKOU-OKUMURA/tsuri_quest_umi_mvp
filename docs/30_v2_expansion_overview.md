...
作成日: 2026-07-06
状態: 仕様確定・V2実装進行中（E0〜E6・E10、docs/35〜44の完了分、Release Gate 0/1、ID-01、REL-01最小export、SAVE-01〜04・UI-QUEST-01・UI-READY-01・docs/33 M0/C0、QA-RELEASE骨格、DOCS-RELEASE草案、E7-coreを前提化。E7-fight / E7-UI、E11、RIGHTS-01A、TARGET-01 / SIGN-01、固定RC検証は未完）
...
残り作業のロードマップ（2026-07-11、リリース前横断監査docs/45と是正進捗を反映）:
...
2. **Release Gate 1（2026-07-11完了）** — SAVE-01の将来版セーブ保護、SAVE-02の保存失敗伝播、SAVE-03の意味検証付きbackup選択、SAVE-04の旧依頼repairを完了。次の本線はE7
3. **E7（core完了、fight / UI着手可）** — 「難易度=新規セーブ時のみ」（決定#3）のためローンチ前必須。倍率・保存・監査APIを先行統合済み。core commitを起点にfight / UIを並列実装し、現行3スロットの上書き安全性も同時確認
4. **E11** — 設定・入力・表示・製品外装を完成。docs/33 M0/C0と依頼/READYの局所P1は完了済み
5. **Pre-RC製品文書最終同期 → 固定RC検証 ＋ 性能/soak ＋ 3難易度通し確認** — release verifier骨格と製品文書草案は完了済み。E7 / E11 / 外部判断を反映してREADME等をPre-RCでcloseしてからRCを固定し、全smoke/audit、未説明ERROR / WARNING、clean export、1280x720・60fps目標と30分soak、序盤・中盤・終盤の人手受入を行う
6. **=== ローンチ ===**（スコープは決定#18）
...
- 意味破損fixture: `{}`、`{"unknown_only": true}`、`{"version": 1, "level": {}}`、`{"version": 1, "money": -1}`、`{"version": 1, "money": 0.5}`、`{"version": 1, "money": 1e100}`、`{"version": 1, "play_seconds": null}`、`{"version": 1, "inventory": []}`、`inventory` / `caught_counts` / `eaten_recipes`の各valueまたは全value総和がJSON安全整数上限を超えるもの、`{"version": 1, "spot_caught_counts": {"harbor_pier": []}}`、`{"version": 1, "spot_caught_counts": {"harbor_pier": {"aji": -1}}}`、`{"version": 1, "quest_board": ["broken"]}`
...
| E7 | **実装中（core完了）** | E7-coreの倍率・save契約・監査を統合済み。同commitを起点にfight / UIを並列実装し、core→fight→UIの順で統合 |
...
| RIGHTS-01A | **外部証拠待ち** | `[RIGHTS-01A]=pending`。`docs/31`、`LICENSE.md`、`THIRD_PARTY_NOTICES.md`、権利監査scriptは存在。U-01〜U-08のうち出所・入力権利・icon・商標・権利者名をRC固定前にcloseする |
...
| QA-RELEASE | **骨格完了** | scene 29件＋script-only audit 1件をmanifest完全一致で列挙。run固有HOME / logs、source・manifest開始終了hash、未知ERROR / WARNING拒否、RC成果物再hashを実装。E7-coreでdifficulty auditを追加して30対象へ更新し、固定RCの`--rc`実行は未完 |
| PERF-RELEASE | TARGET-01とRC待ち | 基準機・計測法の固定後、重い代表状態と30分soak |
| DOCS-RELEASE | **草案完了** | README / VALIDATION / CHANGELOG / MANIFESTを現行Pre-RCとQA-RELEASE最終APIへ同期。E7 / E11 / RIGHTS / TARGET / SIGN確定後に再同期し、RC固定後は内容を変えない |
...
| QA-RELEASE（release verifier骨格） | 完了 | 2026-07-11 | E7 difficulty audit追加後の30対象を自動列挙＋manifest完全一致で管理。特殊3件presence、process-group timeout、run固有HOME / logs、source・manifest安定性、未知WARNING拒否、RC証跡開始終了再hashを実装。固定RC evidenceは未入力 |
| DOCS-RELEASE（製品文書草案） | 草案完了 | 2026-07-11 | README / VALIDATION / CHANGELOG / MANIFESTを現行機能・Pre-RC境界・QA-RELEASE APIへ同期。E7 / E11 / 外部判断を反映する固定RC前の最終再同期（close）は継続 |
| E7 | 実装中 | — | ローンチ前。2026-07-11にcoreを先行統合。fight / UIを同commitから並列実装し、core→fight→UIで統合 |
| E11 | 一部着手 | — | docs/31 素材台帳、Release Gate 0、ID-01、REL-01最小exportは完了。設定等の画面実装はE7後 |
...
| docs/45 リリース前監査フォローアップ | 是正中 | **現在の最優先** | Release Gate 0/1・ID-01・REL-01最小export・SAVE-01〜04・UI-QUEST-01・UI-READY-01・M0・C0・QA-RELEASE骨格・DOCS-RELEASE草案は完了。E7、E11、RIGHTS / TARGET / SIGN、固定RC検証が残る。詳細briefはdocs/45 |
