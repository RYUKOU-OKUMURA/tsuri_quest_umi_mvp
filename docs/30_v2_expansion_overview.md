...
4. **E11（進行中）** — SETTINGS-AUDIO、SLOT-DELETE UI、DISPLAY、INPUT-COMMON、画面別INPUT第1〜4roundは完了。入力baselineは42件から4件へ減少し、完了済み12画面は0件。残りはSHARK_PENの画面別INPUT、EXTERIOR、最終受入。docs/33 M0/C0と依頼/READYの局所P1は完了済み
...
| Wave 4B: 画面別入力 | INPUT-COMMON統合済みtipから、監査で失敗した画面を1画面1brief・最大3並列で修正。第1〜4roundは完了し、STATUS / COOKING / QUEST_BOARDも0件へ収束。次はSHARK_PEN単独round。`title` / `settings` はspine ownerが保持 | **E11 Gate**: 設定・削除・表示・全画面入力・外装・最小export回帰がgreen |
...
| E11実装 | **進行中** | SETTINGS-AUDIO、セーブ枠削除backend/UI、DISPLAY、INPUT-COMMON、画面別INPUT第1〜4roundを完了。完了済み12画面を0件へ収束し、最新baselineは4件（P1 3 / P2 1）。次はSHARK_PENを単独で修正し、owner境界とU-04解消後のEXTERIOR、最終受入へ進む |
| QA-RELEASE | **骨格完了** | 画面別input smoke 12本を含む46対象をmanifest完全一致で列挙。run固有HOME / logs、source・manifest開始終了hash、未知ERROR / WARNING拒否、fixture単位の既知警告scope、RC成果物再hashを実装し、skeleton 46対象はgreen。固定RCの`--rc`実行は未完 |
...
| QA-RELEASE（release verifier骨格） | 完了 | 2026-07-11 | 画面別input smoke 12本を含む46対象を自動列挙＋manifest完全一致で管理。セーブ2件・settings smoke 1件・export 1件の特殊runner 4件presence、process-group timeout、run固有HOME / logs、source・manifest安定性、未知WARNING拒否、fixture単位の既知警告scope、RC証跡開始終了再hashを実装。skeleton 46対象はgreen、固定RC evidenceは未入力 |
...
| E11 | 進行中 | — | SETTINGS-AUDIO、セーブ枠削除backend/UI、DISPLAY（fullscreen保存・起動時適用・keep＋黒帯の実runtime matrix QA）、INPUT-COMMON、画面別INPUT第1〜4roundを完了。残りはSHARK_PENの4件、EXTERIORと最終受入 |
...
| docs/54 UIブラッシュアップ総合計画 | 現行 | I1第4round完了、次はSHARK_PEN単独 | 完了済み12画面を0件へ収束。SHARK_PEN単独→C1-B/R5-B/FIGHT authored 3並列を定義 |
