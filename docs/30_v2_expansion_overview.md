...
4. **E11（進行中）** — SETTINGS-AUDIO、SLOT-DELETE UI、DISPLAY、INPUT-COMMON、画面別INPUT第1・第2round（TITLE / FISHING / SHIPYARD / SETTINGS / FISHING_SPOTS / SHOP）は完了。入力baselineは42件から24件へ減少。残りは画面別INPUT修正、EXTERIOR、最終受入。docs/33 M0/C0と依頼/READYの局所P1は完了済み
...
| Wave 4B: 画面別入力 | INPUT-COMMON統合済みtipから、監査で失敗した画面を1画面1brief・最大3並列で修正。第1round TITLE / FISHING / SHIPYARD、第2round SETTINGS / FISHING_SPOTS / SHOPは完了。次はHARBOR / FISH_BOOK / MARKET。`title` / `settings` はspine ownerが保持 | **E11 Gate**: 設定・削除・表示・全画面入力・外装・最小export回帰がgreen |
...
| E11実装 | **進行中** | SETTINGS-AUDIO、セーブ枠削除backend/UI、DISPLAY、INPUT-COMMON、画面別INPUT第1・第2roundを完了。完了済み6画面を0件へ収束し、最新baselineは24件（P1 18 / P2 6）。次はHARBOR / FISH_BOOK / MARKETを3並列で修正し、owner境界とU-04解消後のEXTERIOR、最終受入へ進む |
| QA-RELEASE | **骨格完了** | 画面別input smoke 6本を含む40対象をmanifest完全一致で列挙。run固有HOME / logs、source・manifest開始終了hash、未知ERROR / WARNING拒否、fixture単位の既知警告scope、RC成果物再hashを実装し、skeleton 40対象はgreen。固定RCの`--rc`実行は未完 |
...
| QA-RELEASE（release verifier骨格） | 完了 | 2026-07-11 | 画面別input smoke 6本を含む40対象を自動列挙＋manifest完全一致で管理。セーブ2件・settings smoke 1件・export 1件の特殊runner 4件presence、process-group timeout、run固有HOME / logs、source・manifest安定性、未知WARNING拒否、fixture単位の既知警告scope、RC証跡開始終了再hashを実装。skeleton 40対象はgreen、固定RC evidenceは未入力 |
...
| E11 | 進行中 | — | SETTINGS-AUDIO、セーブ枠削除backend/UI、DISPLAY（fullscreen保存・起動時適用・keep＋黒帯の実runtime matrix QA）、INPUT-COMMON、画面別INPUT第1・第2roundを完了。残りは24件の画面別INPUT修正、EXTERIORと最終受入 |
...
| docs/54 UIブラッシュアップ総合計画 | 現行 | I1第2round完了、次は第3round | 完了済み6画面を0件へ収束。次のHARBOR / FISH_BOOK / MARKET 3並列→C1-B/R5-B/FIGHT authored 3並列を定義 |
