...
| E11実装 | **進行中** | SETTINGS-AUDIO、セーブ枠削除backend、入力・解像度QA harnessを完了。settings spineはセーブ枠削除UI → DISPLAY → INPUT-COMMON → EXTERIORの順で継続 |
...
| E11 | 進行中 | — | SETTINGS-AUDIO（設定画面・BGM/SE保存・タイトル/港導線）、セーブ枠削除backend、入力・解像度QA harnessを完了。残りはセーブ枠削除UI、DISPLAY、INPUT-COMMON、EXTERIORと最終受入 |
