...
- 追記: 魚行は調理専用の小アイコンではなく、`assets/showcase/underwater/fish/*_card_portrait.png` を優先し、水中ファイト基準の魚アート密度をリスト内でも使う。数量欄は右側に確保し、`× 12 匹` などが折れずに読める状態を維持する。表示順は調理画面用に `アジ → サバ → マダイ → カサゴ → ヒラメ → カワハギ` を優先し、赤身・根魚・平たい魚の見た目の幅もcontent/layout audit対象にして、6行密度を維持する。
