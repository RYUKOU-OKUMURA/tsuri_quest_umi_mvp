...
- 状態: 実行計画 v1。I0、I1第1round（TITLE / FISHING / SHIPYARD）、第2round（SETTINGS / FISHING_SPOTS / SHOP）、第3round（HARBOR / FISH_BOOK / MARKET）は完了。次はI1第4round。進捗の正本は `docs/30_v2_expansion_overview.md` §6、画面ごとの採用値は `docs/qa/<screen>_qa.md` とする。
...
3. I0統合後の入力監査42件を0件にする。第1roundでTITLE / FISHING / SHIPYARDの7件、第2roundでSETTINGS / FISHING_SPOTS / SHOPの11件、第3roundでHARBOR / FISH_BOOK / MARKETの9件を閉じ、最新baselineは15件。入力変更後の実画面を新しいvisual baselineとして固定する。
4. I0統合、RIGHTS-01A U-04 close、`title` / `settings` のowner境界解消後に `E11-EXTERIOR` を開く。V1は前提にせず、残るI1と別ownerで並走できる。
5. Visual Wave V1を `調理C1-B` / `ステータスR5-B` / `水中ファイトのフローティングカード` の3並列で開始する。
6. 調理は `C1-B → C2 runtime採用 → C3 → C4 → C5` を同一レーン内で直列に進め、他画面の独立upliftと並走させる。V2以降はPre-RC cutoff前に明示採用する分だけ統合し、残りは発売後Waveへ送る。

現時点で視覚上の未解決P1はない。一方、I1第3round統合後のfresh隔離HOME実入力probeでは残る4画面に15件（P1 11 / P2 4）の課題が再現しているため、固定RC前の優先順位は視覚P2より入力P1を上とする。素材brief・候補生成などruntime非変更の準備はI1と並走できるが、画面runtimeへの配線と採用は入力baseline固定後に行う。
...
I1第3round統合後のfresh隔離HOME実入力probeでは、TITLE / FISHING / SHIPYARD / SETTINGS / FISHING_SPOTS / SHOP / HARBOR / FISH_BOOK / MARKETが0件となり、残る4画面・15件を再現した。

| 種類 | 件数 | 重要度 |
|---|---:|---|
| 初期focusなし | 4 | P1 |
| disabled操作へfocus到達 | 3 | P1 |
| focus孤立 | 4 | P1 |
...
I1第3round統合後baselineの画面別failure分類は次のとおり。これは着手順を決めるための監査記録であり、I1の各統合後に再計測して上書きする。
...
第1roundの `INPUT-TITLE` / `INPUT-FISHING` / `INPUT-SHIPYARD`、第2roundの `INPUT-SETTINGS` / `INPUT-FISHING_SPOTS` / `INPUT-SHOP`、第3roundの `INPUT-HARBOR` / `INPUT-FISH_BOOK` / `INPUT-MARKET` は完了した。次の第4roundは `INPUT-STATUS` / `INPUT-COOKING` / `INPUT-QUEST-BOARD` とし、最後に `INPUT-SHARK-PEN` を単独roundで処理する。各roundで3件を実装し、実装担当とは別のサブエージェントが結論非開示のread-onlyレビューを行う。親がdiff・実入力・原寸focusスクショを確認し、A → B → Cの順でmainへ直列統合してstrict probeを再実行する。
...
- 専用input smoke 9本をrelease manifestへ登録し、`e11_qa_harness_verify.sh`とrelease verifier 43対象がgreen。最新baselineは42件から15件（P1 11 / P2 4）へ減少し、完了済み9画面はいずれも0件。

### 今すぐ開始するI1第4roundの3セッション

1. `INPUT-STATUS`
2. `INPUT-COOKING`
3. `INPUT-QUEST-BOARD`

第4round完了後は `INPUT-SHARK-PEN` を単独セッションでcloseし、M-INPUTの13画面finding 0を固定する。
