...
- 状態: 実行計画 v1。I0、I1第1round（TITLE / FISHING / SHIPYARD）、第2round（SETTINGS / FISHING_SPOTS / SHOP）は2026-07-15完了。次はI1第3round。進捗の正本は `docs/30_v2_expansion_overview.md` §6、画面ごとの採用値は `docs/qa/<screen>_qa.md` とする。
...
3. I0統合後の入力監査42件を0件にする。第1roundでTITLE / FISHING / SHIPYARDの7件、第2roundでSETTINGS / FISHING_SPOTS / SHOPの11件を閉じ、最新baselineは24件。入力変更後の実画面を新しいvisual baselineとして固定する。
...
現時点で視覚上の未解決P1はない。一方、I1第2round統合後のfresh隔離HOME実入力probeでは残る7画面に24件（P1 18 / P2 6）の課題が再現しているため、固定RC前の優先順位は視覚P2より入力P1を上とする。素材brief・候補生成などruntime非変更の準備はI1と並走できるが、画面runtimeへの配線と採用は入力baseline固定後に行う。
...
I1第2round統合後のfresh隔離HOME実入力probeでは、TITLE / FISHING / SHIPYARD / SETTINGS / FISHING_SPOTS / SHOPが0件となり、残る7画面・24件を再現した。

| 種類 | 件数 | 重要度 |
|---|---:|---|
| 初期focusなし | 6 | P1 |
| disabled操作へfocus到達 | 5 | P1 |
| focus孤立 | 7 | P1 |
...
I1第2round統合後baselineの画面別failure分類は次のとおり。これは着手順を決めるための監査記録であり、I1の各統合後に再計測して上書きする。
...
第1roundの `INPUT-TITLE` / `INPUT-FISHING` / `INPUT-SHIPYARD` と第2roundの `INPUT-SETTINGS` / `INPUT-FISHING_SPOTS` / `INPUT-SHOP` は完了した。次の第3roundは `INPUT-HARBOR` / `INPUT-FISH_BOOK` / `INPUT-MARKET` とする。各roundで3件を実装し、実装担当とは別のサブエージェントが結論非開示のread-onlyレビューを行う。親がdiff・実入力・原寸focusスクショを確認し、A → B → Cの順でmainへ直列統合してstrict probeを再実行する。最後に残る1件は単独roundで処理する。
...
- 専用input smoke 6本をrelease manifestへ登録し、`e11_qa_harness_verify.sh`とrelease verifier 40対象がgreen。最新baselineは42件から24件（P1 18 / P2 6）へ減少し、完了済み6画面はいずれも0件。

### 今すぐ開始するI1第3roundの3セッション
