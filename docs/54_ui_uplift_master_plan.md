...
- 状態: 実行計画 v1。M-INPUT、Visual Wave V1/V2を完了し、M-V1/M-V2を達成。次は`C3` / `SHIPYARD-D0` / `TACKLE-T1`のVisual Wave V3。進捗の正本は `docs/30_v2_expansion_overview.md` §6、画面ごとの採用値は `docs/qa/<screen>_qa.md` とする。
...
6. 調理は `C1-B → C2 runtime採用` を完了した。次は `C3 → C4 → C5` を同一レーン内で直列に進め、他画面の独立upliftと並走させる。V2以降はPre-RC cutoff前に明示採用する分だけ統合し、残りは発売後Waveへ送る。
7. Visual Wave V2の `C2-WIRE` / `MAP-M1` / `FIGHT-A2` を2026-07-17に完了する。MAP-M1は単一素材を無理に採用せず、構成原因を確定して`MAP-D0`へ後続化する。

現時点で視覚上の未解決P1はなく、I1最終round統合後のfresh隔離HOME実入力probeでも13画面すべてfinding 0となった。Visual Wave V2はC2食事シーン背景とFIGHT-A2下段操作盤を採用し、MAP-M1は差分Top1を単一素材でなく構成原因と診断した。各独立レビューP0/P1/P2/P3 0と最終release gateを通過してM-V2を達成したため、次は`C3` / `SHIPYARD-D0` / `TACKLE-T1`を開始する。
...
| 調理 COOK_SELECT / MEAL_RESULT | C0、C1-A厨房背景、C1-B料理カード紙面/タイトル帯、C2食事シーン背景 | 次はC3。C1-A〜C2へ戻らない |
...
| 調理 / `cooking_screen.gd` | C2まで完了 | C3〜C5。一点物素材の密度 | 次はC3、以後専用直列レーン | P2 |
...
`調理C2` は、準備済み候補を同一runtime状態で正式採用し、`meal_scene_bg.png`、専用product processor、旧generator guard、台帳・監査consumer、MEAL_RESULT背景slotをfreezeした。人物・料理・報酬カード・文字・座標と他4状態は不変。次はC3へ進み、C2背景を再オープンしない。
...
| V3（次） | C3 / SHIPYARD-D0 / TACKLE-T1 | 3 | EXP祝祭、船着き場の採用方向とvisual QA、代表商品pilot |
...
C2は同一状態runtime比較を経て採用済み。既存の `player_eating_pose`、料理、カード、freeze座標を動かさず、他4状態もpixel-identicalを確認した。採用背景とproduct processorをfreezeし、次はC3へ進む。
...
V3も1画面1concernを維持する。C3はEXP_GAIN祝祭だけ、SHIPYARD-D0は製品UIを変えず採用方向とvisual QA、TACKLE-T1は代表商品1点のpilotだけを扱う。MAP-D0は採用全画面モック先行の別sliceとし、単一素材slotへ戻さない。
