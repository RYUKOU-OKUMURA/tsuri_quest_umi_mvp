...
- 状態: 実行計画 v1。I0、I1第1〜4roundは完了。次は`INPUT-SHARK-PEN`の単独round。進捗の正本は `docs/30_v2_expansion_overview.md` §6、画面ごとの採用値は `docs/qa/<screen>_qa.md` とする。
...
3. I0統合後の入力監査42件を0件にする。第1roundで7件、第2roundで11件、第3roundで9件、第4roundのSTATUS / COOKING / QUEST_BOARDで11件を閉じ、最新baselineはSHARK_PENの4件。入力変更後の実画面を新しいvisual baselineとして固定する。
...
現時点で視覚上の未解決P1はない。一方、I1第4round統合後のfresh隔離HOME実入力probeではSHARK_PENに4件（P1 3 / P2 1）の課題が再現しているため、固定RC前の優先順位は視覚P2より入力P1を上とする。素材brief・候補生成などruntime非変更の準備はI1と並走できるが、画面runtimeへの配線と採用は入力baseline固定後に行う。
...
I1第4round統合後のfresh隔離HOME実入力probeでは、SHARK_PEN以外の12画面が0件となり、残る1画面・4件を再現した。

| 種類 | 件数 | 重要度 |
|---|---:|---|
| 初期focusなし | 1 | P1 |
| disabled操作へfocus到達 | 1 | P1 |
| focus孤立 | 1 | P1 |
...
I1第4round統合後baselineの画面別failure分類は次のとおり。これは着手順を決めるための監査記録であり、I1の各統合後に再計測して上書きする。
...
第1〜3roundに続き、第4roundの `INPUT-STATUS` / `INPUT-COOKING` / `INPUT-QUEST-BOARD` も完了した。最後に `INPUT-SHARK-PEN` を単独roundで処理する。各roundで実装担当とは別のサブエージェントが結論非開示のread-onlyレビューを行い、親がdiff・実入力・原寸focusスクショを確認してmainへ統合し、strict probeを再実行する。
...
- 専用input smoke 12本をrelease manifestへ登録し、最新baselineは42件から4件（P1 3 / P2 1）へ減少。SHARK_PEN以外の12画面はいずれも0件。

### 次に開始するI1最終round

1. `INPUT-SHARK-PEN`

この単独セッションでSHARK_PENの4件をcloseし、M-INPUTの13画面finding 0を固定する。
