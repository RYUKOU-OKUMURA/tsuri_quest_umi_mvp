...
- 状態: 実行計画 v1。I0とI1第1round（TITLE / FISHING / SHIPYARD）は2026-07-15完了。次はI1第2round。進捗の正本は `docs/30_v2_expansion_overview.md` §6、画面ごとの採用値は `docs/qa/<screen>_qa.md` とする。
...
3. I0統合後の入力監査42件を0件にする。第1roundでTITLE / FISHING / SHIPYARDの7件を閉じ、最新baselineは35件。入力変更後の実画面を新しいvisual baselineとして固定する。
...
現時点で視覚上の未解決P1はない。一方、I1第1round統合後の実入力probeでは残る10画面に35件（P1 27 / P2 8）の課題が再現しているため、固定RC前の優先順位は視覚P2より入力P1を上とする。素材brief・候補生成などruntime非変更の準備はI1と並走できるが、画面runtimeへの配線と採用は入力baseline固定後に行う。
...
I1第1round統合後の隔離HOME実入力probeでは、TITLE / FISHING / SHIPYARDが0件となり、残る10画面・35件を再現した。

| 種類 | 件数 | 重要度 |
|---|---:|---|
| 初期focusなし | 8 | P1 |
| disabled操作へfocus到達 | 7 | P1 |
| focus孤立 | 9 | P1 |
...
I1第1round統合後baselineの画面別failure分類は次のとおり。これは着手順を決めるための監査記録であり、I1の各統合後に再計測して上書きする。
...
- 専用input smoke 3本をrelease manifestへ登録し、`e11_qa_harness_verify.sh`はrelease tests=37でgreen。最新baselineは42件から35件（P1 27 / P2 8）へ減少し、3画面はいずれも0件。

### 今すぐ開始するI1第2roundの3セッション

1. `INPUT-SETTINGS`
2. `INPUT-FISHING_SPOTS`
3. `INPUT-SHOP`
