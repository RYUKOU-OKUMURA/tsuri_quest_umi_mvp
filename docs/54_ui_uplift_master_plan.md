...
- 状態: 実行計画 v1。進捗の正本は `docs/30_v2_expansion_overview.md` §6、画面ごとの採用値は `docs/qa/<screen>_qa.md` とする。
...
1. `E11-INPUT-COMMON` を単一ownerで直列実装する。
2. 共通tipから、入力監査に失敗した画面を1画面1brief・最大3並列で修正する。
3. 入力監査43件を0件にし、入力変更後の実画面を新しいvisual baselineとして固定する。
4. Visual Wave V1を `調理C1-B` / `ステータスR5-B` / `水中ファイトのフローティングカード` の3並列で開始する。
5. 調理は `C1-B → C2 runtime採用 → C3 → C4 → C5` を同一レーン内で直列に進め、他画面の独立upliftと並走させる。
6. V1受入後に `E11-EXTERIOR` を開き、発売GateをP2/P3 backlogより先に進める。V2以降はPre-RC cutoff前に明示採用する分だけ統合し、残りは発売後Waveへ送る。

現時点で視覚上の未解決P1はない。一方、実入力probeでは13画面に43件（P1 34 / P2 9）の課題が再現しているため、固定RC前の優先順位は視覚P2より入力P1を上とする。素材brief・候補生成などruntime非変更の準備はINPUT-COMMONと並走できるが、画面runtimeへの配線と採用は入力baseline固定後に行う。
...
| 調理 COOK_SELECT | C0、C1-A厨房背景 | 次はC1-B。背景へ戻らない |
| ステータス | R5-A中央プレイヤーhero | 次はR5-B。heroは維持 |
| 港 | 司令盤v1.2 | freeze維持 |
| 水上READY / 水中FIGHT | 基盤レイアウト、時間帯・天候、最長餌魚名P1 | 構成freeze。authored質感のみP2 |
...
|---|---|---|---|---|
| タイトル / `title_screen.gd` | E7難易度・slot確認freeze | 正式名、version、icon、splash | INPUT後にE11-EXTERIOR | 発売Gate |
| 港 / `harbor_screen.gd` | 司令盤freeze | 夜専用景観、本物の天候先読み | 新P1なしなら保留 | P3 / feature |
| 釣行 / `fishing_screen.gd` | READY/FIGHT構成freeze | FIGHTのフローティングカードとスリムバーのauthored質感 | V1でカード、V2でバーを別採否 | P2 |
| 釣り場マップ / `fishing_spot_select_screen.gd` | uplift継続中 | 現行実画面とreference/06のTop1を再計測 | Visual V2で1スロット監査 | P2候補 |
| 調理 / `cooking_screen.gd` | C1-Aまで完了 | C1-B〜C5。一点物素材の密度 | 専用直列レーン | P2 |
| 魚図鑑 / `fish_book_screen.gd` | v1採用・魚素材完了 | 現行QA上のP1/P2なし | 回帰のみ | freeze |
| ステータス / `status_screen.gd` | R5-A完了 | 木・真鍮・紙の全画面authored枠 | Visual V1 | P2 |
...
現HEADの隔離HOME実入力probeでは、13画面・43件を再現した。

| 種類 | 件数 | 重要度 |
|---|---:|---|
| 初期focusなし | 11 | P1 |
| disabled操作へfocus到達 | 8 | P1 |
| focus孤立 | 10 | P1 |
| 可視focus style不足 | 3 | P1 |
| focus可能要素なし | 2 | P1 |
| 戻る契約未観測 | 9 | P2 |

この数は実装briefへ固定値として焼き込まない。INPUT-COMMON変更後に再probeし、最新failure manifestから画面別briefを生成する。監査のregistryや分類を緩めて件数だけを減らす修正は禁止する。

現baselineの画面別failure分類は次のとおり。これは着手順を決めるための監査記録であり、I0後に再計測して上書きする。

| 画面 | 現baselineで観測したfailure |
|---|---|
| title | 初期focus、disabled到達、focus孤立 |
| harbor | disabled到達、focus孤立 |
| fishing_spots | 初期focus、disabled到達、focus孤立、可視focus、戻る |
| fishing | focus可能要素、初期focus、戻る |
| cooking | 初期focus、disabled到達、focus孤立、戻る |
| market | 初期focus、disabled到達、focus孤立、戻る |
| shop | 初期focus、disabled到達、focus孤立、可視focus、戻る |
| shipyard | focus可能要素、初期focus |
| status | 初期focus、focus孤立、戻る |
| fish_book | 初期focus、focus孤立、戻る |
| quest_board | 初期focus、disabled到達、focus孤立、戻る |
| shark_pen | 初期focus、disabled到達、focus孤立、戻る |
| settings | 可視focus |

### 3.2 Gate B: 視覚P2

次の3件を最初のVisual Waveとする。
...
いずれも構成・主導線・freeze矩形は維持し、1フェーズ1仮説・1画面1concernで進める。
...
| I0 | E11-INPUT-COMMON共通基盤 | 1 | probe self-test / baseline / strict、13画面registry、validate green |
| I1 | 画面別INPUT修正 | 最大3 | 13画面のstrict finding 0、全screen smoke、マウス回帰green |
| V0 | visual baseline再固定 | 親が直列 | INPUT後の代表/高リスク実画面を保存 |
| V1 | C1-B / R5-B / FIGHT-A1 | 3 | 各Top1が縮小、P1/P2レビュー残件0 |
| X0 | E11-EXTERIOR / releaseレーン開始 | 正本に従う | 正式外装をcloseし、Pre-RC cutoff条件を準備 |
| V2 | C2 runtime / MAP-M1 / FIGHT-A2 | 3 | C2正式採否、Map 1スロット、下段スリムバーの各採否 |
...
I0中に、V1のbrief作成・reference分解・source候補生成だけを別worktreeで準備してよい。ただし `src/ui/**`、製品asset、QA freeze、共通ハブへ触れず、I1完了後のbaselineを使って仮配線・採否をやり直す。

X0はM-V1完了時点で開始でき、V2〜V5やK0を待たない。X0と、cutoff前に採用すると決めたV2以降は別ownerで並走できるが、V2以降はfixed RCの必須条件ではない。X1のPre-RC cutoff後に未統合のvisual branchを同じRCへ追加せず、必要なら発売後の更新として扱う。
...
最初の3セッションは `INPUT-TITLE` / `INPUT-FISHING` / `INPUT-SHIPYARD` とする。各roundで3件を実装し、実装担当とは別のサブエージェントが結論非開示のread-onlyレビューを行う。親がdiff・実入力・原寸focusスクショを確認し、A → B → Cの順でmainへ直列統合してstrict probeを再実行する。最後に残る1件は単独roundで処理する。
...
| M-V1 | C1-B / R5-B / FIGHT-A1採否、各独立レビューP1/P2 0 |
...
`E11-INPUT-COMMON`。共有ハブを持つため並列実装にしない。他2枠を使う場合は `INPUT-FISHING` と `INPUT-SHIPYARD` のread-only事前棚卸しだけに留める。
...
### M-INPUT完了後に開始する3セッション

1. `COOK-C1B`
2. `STATUS-R5B`
3. `FIGHT-A1`

この順で進めれば、発売前の入力P1を先に閉じながら、共有ファイルの競合を抑え、残る視覚P2を最も面積・視線効果の高い3画面から並列に縮められる。
