...
- 状態: 実行計画 v1。I0は2026-07-15完了、次はI1。進捗の正本は `docs/30_v2_expansion_overview.md` §6、画面ごとの採用値は `docs/qa/<screen>_qa.md` とする。
...
3. I0統合後の入力監査42件を0件にし、入力変更後の実画面を新しいvisual baselineとして固定する。
...
現時点で視覚上の未解決P1はない。一方、I0統合後の実入力probeでは13画面に42件（P1 34 / P2 8）の課題が再現しているため、固定RC前の優先順位は視覚P2より入力P1を上とする。素材brief・候補生成などruntime非変更の準備はI1と並走できるが、画面runtimeへの配線と採用は入力baseline固定後に行う。
...
I0統合後の隔離HOME実入力probeでは、13画面・42件を再現した。
...
I0統合後baselineの画面別failure分類は次のとおり。これは着手順を決めるための監査記録であり、I1の各統合後に再計測して上書きする。
...
### 完了した3セッション（2026-07-15）

`E11-INPUT-COMMON`の単一writerと、`INPUT-FISHING` / `INPUT-SHIPYARD` のread-only事前棚卸しを完了した。親レビューと独立レビューを通過し、I0はmainへ統合済み。
