...
- 状態: 実行計画 v1。M-INPUT、Visual Wave V1〜V3を完了し、M-V1〜M-V3を達成。次は発売クリティカルパスの`E11-EXTERIOR`を優先し、Visual Wave V4以降は外装close後に再開判断する。進捗の正本は `docs/30_v2_expansion_overview.md` §6、画面ごとの採用値は `docs/qa/<screen>_qa.md` とする。
...
6. 調理は `C1-B → C2 runtime採用 → C3` を完了した。`C4 → C5` は同一レーン内の直列順を維持し、発売外装close後に再開判断する。
7. Visual Wave V2の `C2-WIRE` / `MAP-M1` / `FIGHT-A2` は2026-07-17に完了した。MAP-M1は単一素材を無理に採用せず、構成原因を確定して`MAP-D0`へ後続化した。

現時点で視覚上の未解決P1はなく、I1最終round統合後のfresh隔離HOME実入力probeでも13画面すべてfinding 0となった。Visual Wave V3ではC3のEXP光背とTACKLE-T1のmarlin詳細大絵を採用し、SHIPYARD-D0は製品UIを変えず4状態visual QAと未採用モックを確立した。レビュー指摘を検査契約まで0件へ収束したためM-V3を達成し、次は新素材を増やすV4より`E11-EXTERIOR`を優先する。
...
| 調理 COOK_SELECT / MEAL_RESULT / EXP_GAIN | C0、C1-A厨房背景、C1-B料理カード紙面/タイトル帯、C2食事シーン背景、C3 EXP光背 | C1-A〜C3へ戻らない。次は外装close後にC4を再開判断 |
| ステータス | R5-A中央プレイヤーhero、R5-B木・真鍮・紙shell | R5-A/R5-Bともfreeze維持 |
| 港 | 司令盤v1.2 | freeze維持 |
| 水上READY / 水中FIGHT | 基盤レイアウト、時間帯・天候、最長餌魚名P1、FIGHT-A1右上カード、FIGHT-A2下段操作盤 | A1/A2ともfreeze維持。新P1がなければ回帰のみ |
...
| 釣行 / `fishing_screen.gd` | READY/FIGHT構成freeze、FIGHT-A1/A2完了 | 現行QA上のP1/P2なし | A1カード/A2下段操作盤を回帰のみ | freeze |
| 釣り場マップ / `fishing_spot_select_screen.gd` | MAP-M1診断完了 | map面積、marker/chip/lock密度、右詳細域の構成差 | 単一素材を採用せず、採用全画面モック先行のMAP-D0 | 設計待ち |
| 調理 / `cooking_screen.gd` | C3まで完了 | C4〜C5。一点物素材の密度 | 外装close後にC4から再開判断 | P2 |
| 魚図鑑 / `fish_book_screen.gd` | v1採用・魚素材完了 | 現行QA上のP1/P2なし | 回帰のみ | freeze |
| ステータス / `status_screen.gd` | R5-A/R5-B完了 | 現行QA上のP1/P2なし | 回帰のみ | freeze |
| 釣具店 / `shop_screen.gd` | T1 marlin pilot採用 | 残り10商品と仕掛け6商品の詳細大絵 | pilot契約のbatch化は発売後判断 | P2候補 / 後続 |
| 船着き場 / `shipyard_screen.gd` | v1、帰港導線統一、D0観測基盤完了 | 未採用モックの製品化判断 | ユーザー承認まで製品不変 | 承認待ち |
...
`調理C2` は、準備済み候補を同一runtime状態で正式採用し、`meal_scene_bg.png`、専用product processor、旧generator guard、台帳・監査consumer、MEAL_RESULT背景slotをfreezeした。C3は`exp_burst_frame`だけを採用し、人物・料理・報酬カード・文字・座標と他4状態は不変。C2/C3を再オープンしない。
...
Visual Wave V2も2026-07-17に完了した。`C2-WIRE`は食卓・ランタン・港の夕景窓を持つMEAL_RESULT背景を採用し、`FIGHT-A2`はFIGHT時だけ下段140pxを一体の木・真鍮・濃紺操作盤へ置換した。`MAP-M1`は原寸・320×180・grayscale診断により、Top1を単一素材でなくmap面積と情報密度の構成原因と判定し、製品/runtime/freezeを変更せず`MAP-D0`へ送った。3レーンの独立レビューはP0/P1/P2/P3 0で、U-08 pendingは維持する。
...
| V2（完了） | C2-WIRE / MAP-M1 / FIGHT-A2 | 3 | C2/FIGHT-A2採用、Mapは構成原因を確定してMAP-D0へ後続化、全レビュー残件0 |
| V3（完了） | C3 / SHIPYARD-D0 / TACKLE-T1 | 3 | C3/T1採用、船着き場は製品不変で4状態QAと未採用方向モックを固定、全レビュー残件0 |
...
| C2-WIRE（完了） | 準備済み食事シーン候補を背景slotへ仮配線 | C1-B統合 | 既存人物・料理・報酬カードを維持し、食事payoffが先に読める |
| C3（完了） | EXP_GAIN祝祭 | C2採否完了 | `exp_burst_frame` 1スロットを採用し、他4状態と既存freezeを維持 |
| C4 | LEVEL_UP祝祭 | C3完了 | Lv遷移と解放報酬がフロー最強のhierarchy |
| C5 | STATUS_SUMMARYアート | C4完了 | R5-Aとの重複責務を避け、5カードを短時間で読める |

C2は同一状態runtime比較を経て採用済み。C3では`exp_burst_frame`だけをsource→決定的processor→productへ移行し、他4状態をpixel-identicalで維持した。C4/C5は発売外装close後の別sliceとする。
...
- `MAP-M1`（完了）: 現行実画面・reference/06・縮小/grayでTop1を再計測し、単一素材では解消できない構成原因と確定した。製品/runtime/freezeは変更せず、採用全画面モックを先に作る`MAP-D0`へ送る。
- `FIGHT-A2`（完了）: FIGHT-A1採用後の画面をbeforeとし、FIGHT時の下段スリムバーだけをauthored素材化して採用・freezeした。READYと中間4状態、カード、入力矩形、動的値は不変。
- `SHIPYARD-D0`（完了）: 製品UIを変えず、未採用「船舶司令盤」モック、代表/高リスク4状態、`shipyard_visual_qa.sh`を確立した。製品化はユーザー承認後の別slice。
- `TACKLE-T1`（完了）: marlinの詳細大絵1セルだけをpilot採用し、非対象10セルのbaseline hash検査と購入後focus証拠を固定した。batch化は発売後判断。
...
| M-V2（2026-07-17達成） | C2/FIGHT-A2採用、MAP-M1構成診断、各独立レビューP0/P1/P2/P3 0、統合visual/smoke/save/E11/validate/release gate green |
| M-V3（2026-07-18達成） | C3 EXP光背 / T1 marlin pilot採用、SHIPYARD-D0観測基盤・未採用モック固定、各独立レビュー指摘0、統合visual/smoke/save/validate green |
...
### 完了したVisual Wave V2（2026-07-17）

- 共通baselineをcommit `8da6d069`へ固定後、`C2-WIRE` / `MAP-M1` / `FIGHT-A2`を3つの独立worktreeで進行し、C2 → MAP → FIGHTの固定順でrebase / mergeした。
- C2はMEAL_RESULT背景を正式採用し、FIGHT-A2はFIGHT専用下段140px操作盤を正式採用した。MAP-M1は構成原因を確定し、素材を無理に採用せず`MAP-D0`へ後続化した。
- 実装者とは別のread-only reviewerと親のdiff・実画像確認を通し、3レーンともP0/P1/P2/P3 0。正式evidence、visual QA、smoke、save、E11 harness、validate、release gateをgreenにしてM-V2を達成した。U-08 pendingは維持する。

### 次に開始するセッション

発売クリティカルパスの`E11-EXTERIOR`を優先する。C4 / QUEST-Q2 / SHARK-S2のVisual Wave V4、TACKLE残batch、船着き場の製品化、MAP-D0は外装close後に再開判断し、いずれも1画面1concernを維持する。
