...
| E11-7 未決5件の決定 | **docs/45 Release Gate 0として即時** | export、設定、入力、表示、製品外装のスコープを先に固定する |
...
## E11-7. 未決事項（ユーザー決定待ち。docs/45 Release Gate 0）

| # | 項目 | 選択肢 | 決定期限 |
|---|---|---|---|
| 1 | 販売チャネル | Steam / itch.io / 両方 /（モバイルは非推奨） | **期限到来。E11着手前** |
| 2 | 対象OS / export形式 | macOS / Windows / 両方など | **最小export spike前** |
| 3 | ゲームパッド対応 | 対応 / 非対応（明示） | **期限到来。E11着手前** |
| 4 | 非16:9の表示方針 | expand維持＋追加QA / keep＋黒帯 | **期限到来。E11着手前** |
| 5 | 正式名称 | 製品表示名、短縮表記、version表記 | **Gate 0で決定。config/nameへの反映はuser data namespace固定後** |

セーブスロット数は決定#17の3スロットで解消済み。決定したらdocs/30 §3-1の決定表へ追記し、本docの該当節を確定させる。user data namespace / OS application ID / store App IDは正式名称と分離して技術判断を記録する。
