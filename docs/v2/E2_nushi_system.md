...
- 通常7釣り場のヌシデータ、出現条件、初回報酬、釣行気配、港ヒント、魚図鑑ヌシ表示を実装済み。
- `danger_reef` の白帝はE4対象として分離済みで、E2完了範囲には含めない。
- 完了確認: `fight_envelope_audit`, `nushi_encounter_audit`, 魚図鑑/市場/釣行系QA, `./tools/validate_project.sh`。
...
- 成立条件の目安: 自動監査では、適正装備・適正Lvの所要時間が子供の集中力の範囲（1ファイト2〜3分以内）に収まり、低装備帯では最大ケースが100%勝利に張り付かないことを確認する。手動プレイ勝率の本調整はE7難易度で扱う
