...
- `record_catch()` の記録更新情報、称号カタログ/判定、称号獲得通知、ステータス画面の称号表示を実装済み。
- E2/E3/E10用の称号条件は、未実装フェーズでは偽になる前提でカタログへ登録済み。
- 完了確認: `status_smoke`, `catch_fanfare_smoke`, 称号関連smoke, `./tools/validate_project.sh`。
