...
- 消費フロー・レア度耐久チャージ・READY UIの正本は `docs/38_shark_bait_ready_selector_spec.md`（docs/30 決定#12 2026-07-07改訂）。要約: 危険海域の釣行中READY状態で餌魚を選択し、`投げる` 押下時に消費する。レア度別の耐久チャージ（コモン1回/アンコモン2回/レア3回/ぬし5回）で、1匹消費するとその回数分のキャストに使える。港の餌魚ボタンは初期選択のみ（消費しない）。ボウズでもチャージ・消費は戻らない
