...
状態: 完了（2026-07-06実装、2026-07-07にE10-4の消費モデルをdocs/38で改訂）。進行状況の正本は docs/30 §6
...
- 消費フロー・レア度耐久チャージ・READY UIの正本は `docs/38_shark_bait_ready_selector_spec.md`（docs/30 決定#12 2026-07-07改訂）。要約: 危険海域の釣行中READY状態で餌魚を選択し、`投げる` 押下時に消費する。レア度別の耐久チャージ（コモン1回/アンコモン2回/レア3回/ぬし5回）で、1匹消費するとその回数分のキャストに使える。港の餌魚ボタンは撤去済み（2026-07-09、餌魚選択はREADYセレクタに一本化。コミット 93e2bc31）。ボウズでもチャージ・消費は戻らない
- 効果（pure関数 `shark_lure_weights(bait_fish_data) -> Dictionary` で重み補正を返し、`encounter_weights()` の `extra_fish_weight_modifiers`（E6で増設済み）に通す）:
  - 餌魚が好物に該当するサメ: 重み **×3.0**
  - 大型2種（shumokuzame / hohojirozame）: 好物の餌魚を消費したキャスト**のみ**抽選テーブルに乗る（基礎重み 0.6 / 0.5）
  - 餌魚なしのキャスト: 通常4種＋レア3種のみ（E4-1 の重みどおり）
...
- 演出フロー:
  ...
  2. 危険海域で**ヌシ級の餌魚**（E10-2 の好物述語: nushi_* または sell 3,000以上）を消費したキャストで、ヒットごとに確率 `MEGALODON_ENCOUNTER_CHANCE := 0.08` で出現
