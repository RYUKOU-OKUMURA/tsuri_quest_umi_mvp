...
| READY餌魚名固定スロット | 3 | 左右矢印を42px→34pxへ圧縮し、`xN` をカード右端固定から魚の右隣の左固定へ変更 | 採用・close。長名や3桁在庫で重心が揺れるP1再発時以外、この系統の位置調整は行わない |
...
- READYセレクタ内の長い魚名（例: `港のぬし・大岩クロダイ`）は魚ポートレート右の2行固定スロットに収める。カード外へのはみ出し、末尾省略、または魚名・在庫数の横幅変化でカード全体が揺れて見える状態はP1として扱う。名前スロットの数値微調整は3回到達済みのため、以後はP1再発時もカード構成を再設計する。
...
差分Top1:
- P1-1: 旧READY名前スロットは `港のぬし・大岩クロダイ` を1行の末尾詰めで表示しており、docs/19 §2.1の通常データ省略ゼロ条件に反した。

再オープン理由:
- docs/45 UI-READY-01のP1再発。READY下段バー全体・FIGHT・抽選/消費ロジックのfreezeは維持し、名前スロット構造だけを再オープンした。

変更したもの:
- `src/ui/components/fight_hud.gd`: 魚ポートレート82px、右側情報スロット、名前2行（16px、必要時14px）・中央揃え、個数74px右下固定、チャージピップ左固定へ再設計。名前の末尾詰めを廃止。
- `tools/fishing_surface_states_preview.gd`: `TSURI_SURFACE_STATE_SHARK_LURE_COUNT` を空文字でも明示0として扱い、`TSURI_SURFACE_STATE_SHARK_LURE_REMAINING` で残チャージを固定再現できるようにした。

変更していないもの:
- READY下段バー3ゾーン、投げるボタン、左右矢印の34px幅、右メニュー、CASTING以降/FIGHTのHUD、餌消費・抽選・魚素材。

状態契約と証拠:

| 状態 | 固定データ | evidence |
|---|---|---|
| 最長名＋x99 | `danger_reef` / `boss_kurodai` / count=99 | `docs/qa/evidence/fishing_surface/2026-07-10_ui_ready_long_name_before.png` / `2026-07-10_ui_ready_long_name_after.png` |
| 通常名＋x101 | `danger_reef` / `medai` / count=101 | `2026-07-10_ui_ready_normal_medai_x101.png` |
| 餌魚なし | `danger_reef` / lure idなし | `2026-07-10_ui_ready_no_lure.png` |
| 残チャージ | `medai` / count=1 / remaining=2 | `2026-07-10_ui_ready_remaining_charge_x1.png` |
| 在庫0＋残チャージ | `medai` / count=0 / remaining=2 | `2026-07-10_ui_ready_zero_stock_remaining2.png` |

採用理由:
- 長名と3桁在庫を同一1280x720状態で省略なく表示し、魚名・個数・ピップがカード外へ出ない。
- 通常名、在庫0、残チャージ、餌魚なしでもカード矩形・左右矢印・主操作の位置を維持する。
- 旧1行末尾詰め案を不採用として固定し、以後の数px微調整ループへ戻らない。

検証:
- `fishing_surface_states_preview.tscn` を非headless通常起動し、5状態のREADY〜BITE連続キャプチャを取得（READY証拠は上表）。
- `fishing_harbor_return_smoke.tscn`: `fishing_harbor_return_smoke: ok`。
- `fishing_spot_select_smoke.tscn`: `fishing_spot_select_smoke: ok`。
- `./tools/fight_visual_qa.sh`: FIGHT runtime capture / 比較画像生成まで終了。
- `./tools/validate_project.sh`: showcase素材参照監査、魚素材重複監査、魚シート監査、Godotロード確認がgreen（GodotのObjectDB snapshot警告は既存環境警告で終了コード0）。
...
差分Top1:
- P1-1: 魚名を画像下へ移した後も、`xN` がカード右端に寄りすぎ、魚画像との間に大きい空白が残って切替時のブレに見える。

スコープ:
- 今回動かしたもの: READY餌魚カード内の左右矢印幅、中央カード幅、在庫数 `xN` の横位置、チャージピップの横位置。
- 触っていないもの: READY下段バーの3ゾーン幅、投げるボタンサイズ、右メニュー、抽選/消費ロジック、CASTING以降のHUDレイアウト。

変更したもの:
- 左右矢印ボタンを42pxから34pxへ圧縮し、中央カードを少し広げた。
- `xN` をカード右端固定の右揃えから、魚の右隣に寄せた左固定表示へ変更した。
- チャージピップも同じ右側スロットに寄せ、空白がカード右端だけへ偏らないようにした。

検証:
- `tools/fishing_surface_states_preview.tscn`: 危険海域READY（`medai` x101 / `boss_kurodai` x99）を取得し、魚と数字の間の空白が縮み、カード外へ出ないことを目視確認。

判断根拠:
- メダイ3桁在庫READY証拠: `docs/qa/evidence/fishing_surface/2026-07-08_ready_lure_tight_medai_x101.png`
- 長名READY証拠: `docs/qa/evidence/fishing_surface/2026-07-08_ready_lure_tight_long_x99.png`

採用理由:
- `x101` が魚の近くに固定され、魚名・在庫数の違いで視線重心が左右へ大きく動きにくくなった。
- 投げるボタンは維持し、先に矢印とカード内余白を詰めるだけで改善できた。
