...
| キーボード入力契約 | 初期focus=`ここで釣る`。有効な`仕掛け切替`/`ここで釣る`/`港へ戻る`/`釣り手帳`/`メニュー`だけを上下左右・Tab/Shift+Tabの閉じたgraphへ接続し、disabled要素は`FOCUS_NONE`で除外。focusは共通4px金枠、Escapeは港遷移exactly once | `src/ui/fishing_spot_select_screen.gd` / `tools/fishing_spot_input_smoke.gd` | ロック・access・仕掛け所持状態は既存値を正とし、状態更新時だけgraphを再評価する |
...
| パラメータ | 回数 | 直近の変更内容 | 状態 |
|---|---|---|---|
| 入力focus収束 | 1 | 共通4px金枠とenabled-only focus graphを採用 | 採用・freeze |
...
（現在作業中のフェーズなし。2026-07-15 のE11入力収束ではfocus graph・共通focus表示・Escape契約だけを追加し、`DETAIL_FRAME_SOURCE_SIZE`、全ウェル座標、`狙い`行、戻るボタン高さ、9サムネイル、素材・font・palette、既存航路/マーカー/spot座標、ロック・access・trip logicは触っていない）
