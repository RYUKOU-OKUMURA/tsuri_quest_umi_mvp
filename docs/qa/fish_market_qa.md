...
最終更新: 2026-07-11 / 状態: v1 freeze・M0空状態P1再修正完了 / RarityStyles共通化済み / R1 Palette確認済み / 帰港導線右下統一済み
...
| 状態 | 判定 | 証拠 |
|---|---|---|
| 通常選択 | M0再修正後も採用 | `docs/qa/evidence/fish_market/2026-07-11_m0_cover_after_select_reference_compare.png` |
| 売却確認 | M0再修正後も採用 | `docs/qa/evidence/fish_market/2026-07-11_m0_cover_after_confirm_reference_compare.png` |
| 売却完了 | M0再修正後も採用 | `docs/qa/evidence/fish_market/2026-07-11_m0_cover_after_sold_reference_compare.png` |
| 空状態 | M0 P1残像ゼロ・採用 | `docs/qa/evidence/fish_market/2026-07-11_m0_cover_empty_before_after.png` / `2026-07-11_m0_cover_after_empty_reference_compare.png` |
...
- M0で空状態はruntime表示として成立し、左紙片・右の金枠/円/波/葉/価格残像は再修正でゼロ化した。氷+木箱の一点物査定トレー、専用市場背景はM1〜M2の素材工事で再検討する。
- 左下の矢印風ボタン枠は `fish_market_backplate.png` に焼き込まれた非クリック装飾。RF4ではruntime「港へ戻る」を右下へ移動し、素材の描き直しはv2候補へ送る。

## 6. フェーズスコープ宣言（作業中のみ）

- M0 P1は再修正まで完了。空状態の左パネル被覆、右詳細の空状態メッセージと通常詳細の表示切替、通常詳細の説明面だけを動かした。論理画面サイズ、一覧7行、上部ステータス、主操作、`RETURN_RECT`、売却仕様、一括売却API、魚価格、確認オーバーレイ、backplate/素材、M1〜M3、他画面は触っていない。
