...
| M2査定トレー | OpenAI生成source + クロマキー除去 + 決定的processor | `MarketIceTrayHero` / `ice_tray_hero.png` | 魚・UI・文字なし。氷+木箱+笹葉だけで待機中の査定台として成立。alpha bbox `(738, 249)–(1118, 366)`、他5レイヤーと全freeze矩形は不動 |
...
- M2一点物2スロットは完了。市場背景と氷+木箱の査定トレーはAI生成source+決定的processorへ移行済み。
- M0の空状態メッセージ面はM2後も表示同値で、左紙片・右の金枠/円/波/葉/価格/トレー残像はゼロ。通常状態復帰もsmokeで維持する。
...
- helperはauto-key border、soft matte、透明threshold 12、opaque threshold 220、despillを使用。透明四隅、部分alpha 2,944px、生成物にマゼンタ縁なしを確認した。
- processorはcutoutのalpha bboxを抽出し、縦横比を保ってsafe-areaへLANCZOS縮小、`DARK_PANEL`相当の10%環境色ティントを適用。製品alpha bboxは `(738, 249)–(1118, 366)`、SHA-256は `046927f6bebdaf3697c45f4a10a03a613e11701a938110eb985c0b2db2810769` で再実行同値。
