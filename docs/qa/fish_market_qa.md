...
最終更新: 2026-07-13 / 状態: v1 freeze・M1分解完了・M2 `market_bg` 採用（`ice_tray_hero` 作業中） / RarityStyles共通化済み / R1 Palette確認済み / 帰港導線右下統一済み
...
- M2 `market_bg` はAI生成一点物へ差し替え済み。残るM2差分は `ice_tray_hero` の氷+木箱の質感だけ。
...
- M2作業中。1スロットずつ `market_bg` → `ice_tray_hero` の順で採否する。動かすものは各AI生成source、決定的processor、当該製品PNG、素材台帳、QA証拠だけ。
- 不動: M1の6レイヤー順序、論理画面サイズ、一覧7行、全freeze矩形、上部ステータス、主操作、`RETURN_RECT`、4状態の情報構成、売却/確認/一括売却ロジック、魚価格、パネル枠3種/header、common/palette/ScreenBase、他画面。M3 CTA/紙面質感へ進まない。
- M2開始時の差分Top3: 1) 市場背景の不在、2) hero査定トレーの氷+木箱の質感、3) CTA/紙面質感（M3へ送る）。`market_bg` はTop1だけ、`ice_tray_hero` はTop2だけを対象にする。
