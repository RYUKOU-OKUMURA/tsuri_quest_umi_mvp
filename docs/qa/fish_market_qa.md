...
最終更新: 2026-07-16 / 状態: v1 freeze・M1/M2/M3完了・INPUT-MARKET完了 / RarityStyles共通化済み / R1 Palette確認済み / 帰港導線右下統一済み
...
| M3主CTA | common追加variant `primary_action_{normal,hover,pressed,focus,disabled}.png` 5状態9-slice / `CART_ACTION_RECT`不変 | `MarketSellBatchButton` | 文字・数字・魚・画面固有意匠のない汎用幾何CTA。runtime文言を維持したまま金色主CTAへ収束。5状態は別texture、focusはcyan輪郭、disabledは濃紺。既存common素材・consumerへの変更なし |
...
| INPUT-MARKET focus契約 | 通常=`選択魚の行`、空=`港へ戻る`、確認=`戻る` | 全enabled行/数量/ページ/一括選択/主CTA/帰港、確認2操作 | disabled/hiddenはfocus graph外。確認中は2操作へtrapし、Escapeは閉じるだけで直前focusを復元。通常Escapeは1回だけ帰港。売却・再入荷後はsemantic identityでA→B→A復帰 |
| 行選択input rect | `Rect2(row.position, Vector2(448, 62))` | `MarketRowSelect_*` | 既存表示・7行座標は不動。数量Button開始x=524より手前へhit/focus領域を閉じ、行focus枠が数量表示を横断せず、mouse hit領域も重複しない |
...
| M3主CTA texture | 1 | common追加variantの文字なし汎用幾何9-sliceを5状態へ分離。content marginを32/7へ収束し実rect 190×50を維持 | 採用・freeze |
...
### 4.4 INPUT-MARKET キーボード状態証拠

| 状態 | 判定 | 証拠 |
|---|---|---|
| 通常選択 | 採用。選択魚の行へ初期focusし、枠は数量欄を横断しない。Tab/矢印で24 enabled操作へ到達しdisabledをskip | `2026-07-16_input_initial_focus.png` |
| 売却確認 | 採用。安全側の「戻る」へ初期focusし、確認2操作へtrap。Escape/Enter後に直前の主CTAへ復帰 | `2026-07-16_input_modal_focus.png` |
| 売却後の空状態 | 採用。disabledの行/一括/主CTAをgraph外にし、「港へ戻る」だけを安全なfallbackにする | `2026-07-16_input_empty_focus.png` |

3枚とも `docs/qa/evidence/fish_market/` の1280×720原寸runtime capture。専用 `market_input_smoke.tscn` は実 `InputEventKey` / `InputEventMouseButton` で、Enter+echo単発、通常Escape帰港1回、modal Escapeはdismissのみ、cancel/sell単発、売却後fallback、empty→再入荷→emptyのfocus graph復帰、mouse選択/数量/売却/帰港を検証する。M1〜M3の既存4状態visual baselineは引き続き非focus状態を正とし、`market_preview.gd`だけがcapture直前にfocusを解除する。製品初期focusと本節の専用input証拠は解除しない。既存4状態、`CART_ACTION_RECT` / `RETURN_RECT`、一覧7行、overlay、M1〜M3素材、価格・save/economyロジックは変更していない。

## 5. 現在の残ギャップ

- P1/P2なし。INPUT-MARKETの基準4件（`INPUT_NO_INITIAL_FOCUS` / `INPUT_DISABLED_REACHED` / `INPUT_FOCUS_ISOLATED` / `INPUT_CANCEL_UNOBSERVED`）は2026-07-16のfresh isolated HOME probeで0件へ収束。実入力CTA原寸5状態とcommon昇格後4状態の全画面画素同値も2026-07-14証拠で受入済み。参照のCTAは現行より横幅比が大きいが、`CART_ACTION_RECT` freezeを再オープンする根拠はなくM3対象外のP3とする。
- M0の空状態メッセージ面はM3後も残像ゼロで、空→再入荷→空復帰をsmokeで維持する。

## 6. フェーズスコープ宣言（作業中のみ）

- 現在作業中のP2フェーズなし。M3のcommon所有・操作状態・4状態画素同値・採用根拠は§1・§4.3、INPUT-MARKETのfocus/復帰契約と原寸証拠は§1・§4.4へfreeze済み。
...
2026-07-16 INPUT-MARKET:
- 共通focus契約へ、表示中の行選択/数量、ページ、一括選択、主CTA、帰港を登録。disabled/hiddenをgraph外へ出し、通常は選択行、空は帰港、売却確認は安全側の「戻る」を初期focusとした。
- 売却確認は2操作だけへfocus trapし、Escapeは確認を閉じるだけ、Enterのcancel/sell後は直前のsemantic focusへ復帰する。通常Escapeはechoを含め帰港1回。売却で主CTAがdisabledになる場合は選択行、全売却時は帰港へfallbackする。
- 行選択の透明hit/focus領域だけを幅510→448へ閉じ、数量Buttonとの重複とfocus枠の文字横断を解消した。行の表示座標・高さ・7行数、`CART_ACTION_RECT` / `RETURN_RECT`、overlay、素材、価格・save/economyは不動。
- M1〜M3の4状態visual baselineは非focus、INPUT-MARKETの初期/modal/empty証拠はfocusありとして責務を分離した。previewの4状態capture直前だけfocusを解除し、製品初期focusとCTA 5状態captureは維持する。
- `market_input_smoke.tscn`、既存`market_smoke.tscn`、4状態+CTA 5状態の`market_visual_qa.sh`、`save_system_verify.sh`、fresh isolated HOMEのE11 probe（市場0件）、`validate_project.sh`、`git diff --check`はgreen。1280×720原寸3状態を§4.4へ保存した。E11 harnessのprobe群は完走し、最終manifest照合だけが親ownerの新smoke登録前を正しく検出した。
