...
最終更新: 2026-07-04 / 状態: backplate/runtime座標再同期済み v1 freeze中
...
| 商品名/状態プレート | 商品名はカード上部プレート中央、価格/所持/装備中/Lv不足は下部プレート中央 | `shop_screen.gd` / `CARD_NAME_OFFSET` / `CARD_STATUS_OFFSET` | 個別Rect管理による左ズレ再発を防ぐ |
| 詳細表示 | 右紙面上にruntime表示 + 選択商品詳細大絵 | `shop_screen.gd` / `shop_detail_item_sheet.png` | 商品名、状態、説明、性能、対応エサ、購入/装備文言は焼き込まない。詳細大絵は商品IDで差し替える |
...
なし。竿/仕掛けの再同期後比較画像は `docs/qa/evidence/tackle_shop/2026-07-04_tackle_shop_rod_resynced_compare.png` / `docs/qa/evidence/tackle_shop/2026-07-04_tackle_shop_rig_resynced_compare.png` に保存済み。広いviewport確認は `docs/qa/evidence/tackle_shop/2026-07-04_tackle_shop_rod_resynced_expanded.png` / `docs/qa/evidence/tackle_shop/2026-07-04_tackle_shop_rig_resynced_expanded.png` に保存済み。

## 5. 現在の残ギャップ

- **P2**: 詳細大絵は既存backplateからの透過切り抜きでv1採用。看板品質へ上げる場合は、11商品分の専用高解像度詳細絵を別素材フェーズで作る。
...
- 実装後比較: `docs/qa/evidence/tackle_shop/2026-07-04_tackle_shop_rod_resynced_compare.png` / `docs/qa/evidence/tackle_shop/2026-07-04_tackle_shop_rig_resynced_compare.png`。
- 広いviewport証拠: `docs/qa/evidence/tackle_shop/2026-07-04_tackle_shop_rod_resynced_expanded.png` / `docs/qa/evidence/tackle_shop/2026-07-04_tackle_shop_rig_resynced_expanded.png`。
