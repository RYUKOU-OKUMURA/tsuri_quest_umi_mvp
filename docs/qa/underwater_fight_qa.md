...
- 2026-07-17: `FIGHT-A2`最終独立レビューP2を解消。processorのdecoded同値判定から暗黙RGBA変換とRGB-only比較を除き、actualが厳密にRGBA 1280×140px、alpha=(255,255)、期待製品とdecoded RGBA完全一致する場合だけcheck green / PNG bytes保持とした。隔離self-testはRGB mode、alpha 1px drift、size、RGB 1px driftを全てcheckでrejectし、同directory atomic生成でRGBA製品へ修復する。decoded同一RGBAのbytes保持、保存失敗時の旧output保全と一時ファイルcleanupも同時検証する。製品/source/runtime/evidenceは不変。

- 2026-07-17: `FIGHT-A2`統合再捕捉で、`process_frame`だけのSubViewport readbackがMetal上で1状態前のWAITING面をAPPROACHへ保存し得るP2を再オープンし解消。決定fixtureは各captureで期待state、固定depth（0/4/7/12/18m）、中間status（投入中/探索中/魚影あり/食いついた）を検査し、surface/HUD/sidebar/status ownerを明示redraw後に`RenderingServer.force_draw(false)`を2回実行、連続readback一致と直前状態HUD probe非一致を満たす画像だけを保存する。builderもbefore/after双方で隣接状態のHUD probe重複をstaleとしてrejectし、隔離self-testへWAITING画像をAPPROACHへ差し替えるnegative probeを追加した。同一fixtureをbase `8da6d069` / TIPへ適用した2連続fresh捕捉は、各runのREADY / CASTING / WAITING / APPROACH / BITEが全て0px、base再実行間とTIP再実行間も全て0px。headless dummy rendererは画像取得不可を即時errorにして終了し、描画待ちで停止しない。製品素材、runtime、正式evidence画像は不変。

- 2026-07-17: `FIGHT-A2`を採用・freeze。FIGHT-A1採用後をbeforeに、FIGHT時の下段140px背景だけを文字なし木・真鍮・濃紺の一体操作盤 `fight_slim_bar_frame.png` へ置換した。原寸before/afterの差分はHUD実Rect `(6,574)-(1274,714)` 内だけで外側0px、focus ringはbase/TIPとも `(443,628)-(636,692)` を維持。独立レビューではbase `c1245689` / TIPに同一の決定fixtureを適用し、`sunny_calm` / `daytime`、魚ID=`aji`の同一魚辞書、表示サイズ24.5cm、各動的描画値を固定したfresh captureで、READY / CASTING / WAITING / APPROACH / BITEの5全画面がpixel-identical（各0px）。builderは5ラベル各1件を必須とし、隔離copyの欠落状態/1px破損self-testを`validate_project.sh`からread-only実行する。通常、`巻く` / `糸を出す` focus、両press、テンション0.94＋魚体力8%＋長い魚名/rarity/行動文、アラ右端寄りでP1なし。中央主操作が先に読め、左右ゲージと競合せず、320×180でreference/14の一体操作盤との差が縮小した。processorはsourceだけを固定加工し、read-only check、decoded pixel同値bytes保持、same-directory atomic replace、隔離RGB破損self-testを備える。FIGHT-A1カード、`fight_hud_frame.png`、runtime文字/数値/18分割ゲージ/input hit rect、上部、水中背景、魚、line/lure、modal/fanfare、進行ロジックは不変。検証: `fight_visual_qa.sh`、`surface_weather_visual_qa.sh`、5状態base/TIP pixel回帰、`fishing_reveal_smoke`、`fishing_harbor_return_smoke`、`fishing_input_smoke`、`catch_fanfare_smoke`、`validate_project.sh`、`git diff --check` green（既知の終了時ObjectDB/resource警告のみ）。証拠: `2026-07-17_fight_a2_{standard_before_after,after_reference_320x180,bar_before_after_reference,standard_after,focus_after,give_line_focus,reel_pressed,give_line_pressed,high_tension_low_stamina_long,ara_right_edge}.png`。素材の第三者権利clearanceは台帳どおりU-08待ちを維持する。
