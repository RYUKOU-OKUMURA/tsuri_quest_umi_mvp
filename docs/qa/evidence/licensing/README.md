...
| U-03 | 各 `assets/showcase/` generate外PNGの生成サービス・生成日・作成者申告 | 対象ファイル一覧に推定が残らず、OpenAI以外のサービス利用も記録される |
...
| U-07 | 販売チャネルと対象OS | Steam AI Content Survey、年齢質問票/IARC、最終notice同梱検証の要否を確定 |
| U-08 | AI入力に第三者著作物・音源を用いたかの作成者申告 | サービス規約上必要な入力権利の確認を完了 |

## チャネル決定後の提出・保存待ち

- Steam採用時: OpenAI画像とSuno音源をPre-Generated AIとしてContent Surveyへ申告し、提出控えを保存する。ゲーム内のlive生成は現状なし。
- 対象ストアの年齢質問票を実内容に基づき回答し、提出控え・発行ratingを保存する。IARCの要否はチャネル決定後に公式手順で確認する。
