...
1. 上部ヘッダー
   - `釣り場を選ぶ`
   - プレイヤーLv、装備竿、現在の出航先、所持金
   - 右側のステータス群は個別カードの羅列ではなく、共通台座の上に並ぶ航海情報バーとして見せる。
...
3. 右詳細パネル
   ...
   - `ここで釣る` / `港へ戻る`
4. 下部達成度台帳
   - 総合達成度と現在の出航先を見出しチップで表示する。
   - 釣り場別の捕獲済み魚種数を `featured_fish` ベースで表示する。
   - 個別カード列ではなく、共通の台帳面を区切ったセルとして見せる。各セルは釣り場名、記録数、達成率、必要Lvを主表示にする。
   - 地図マーカーと同じ選択状態・ロック状態に同期する。ただし選択操作の代替に見えないよう、選択中表示は薄いハイライトと罫線に留める。
   - 右端の調査メモは台帳と同じ紙面フレーム内に置き、現在選択中の釣り場の達成度と未記録魚を補足する。
