...
作成日: 2026-07-10
状態: 採用モック v1.2（目撃談可変高＋ヘッダー基準線補正）Godot 実装・freeze 完了
...
左盤の主要スロットは、最優先魚 `(60,172,364,154)`、副候補 `(436,172,180,154)` / `(628,172,180,154)`、出港情報3枚 `(60,364,270,78)` / `(338,364,270,78)` / `(616,364,192,78)`、食事効果 `(60,532,748,36)`、時間帯3ボタン `(132,576,216,44)` / `(356,576,216,44)` / `(580,576,228,44)` とする。目撃談は食事効果あり `(60,450,748,78)`、なし `(60,450,748,114)` とし、非表示になった食事効果36pxを回収する。拡大時は内部の見出し／アイコン／本文を18px下げて中央配置する。時間帯は左盤下端から12pxを固定し、食事効果の有無で移動させない。

上部は外形 `(32,24,1216,80)` と縦区切りを維持する。場所名／副題はtop内 `(36,3)` / `(38,41)`、共通 `PlayerStatusBar` 港モードのlocal yは Lv/EXP=`7/38`、装備キャプション/竿名=`8/29`、所持金=`6/27` とする。所持金を不動の基準にして4クラスタの文字画素中心差を0.5px以内に収める。
...
主操作は右上の大型「釣り場へ向かう」1つ。補助操作は依頼ボード、調理場、魚市場、釣具店、船着き場、サメの生簀、ステータス、魚図鑑、タイトルへ戻る、時間帯3種。キーボードfocusを現行契約として保証し、ゲームパッドを採用する場合は同じfocus graphを使う。
...
金縁・紙面・主CTAの質感を `StyleBoxFlat` で代替しない。通常時の面は上記素材を使い、`StyleBoxFlat` は通知丸、理由チップ、細いアクセント、および操作状態を示すoverlayに限定する。全route/timeボタンは hoverで明度差、pressedで暗化し、focusは透明fill＋金色2px枠（主CTAのみ3px枠）を重ねる。キーボードで現在focusが常時判別できることを現行契約とし、ゲームパッド採用時は同じ契約で実機確認する。
...
- 出港情報3枚／目撃談／食事効果／時間帯が上記矩形に収まり、食事効果あり／なしで目撃談が78/114pxへ復帰し、時間帯位置が変わらない。
- ヘッダーの場所名、Lv/EXP、装備、所持金が上記基準線にあり、所持金と縦区切りは変化しない。
- 通知バッジ、サメロック説明、推薦優先度、メガロドン前兆、食事効果の表示条件を維持する。
- 全route/timeボタンに可視hover/pressed/focus状態があり、初期focusの主CTAには3px金枠が表示される。

## 4.1 Visual QA契約

`./tools/harbor_visual_qa.sh` は固定previewデータ（乱数なし）で朝まずめ・日中・夜釣りと、ユーザー提示画面に合わせた食事効果なしの日中を撮影する。3時間帯一覧に加えて、採用モックとの日中横並び、グレースケール、各画面320x180（全体640x180）の縮小比較を生成する。before/after比較は固定データ条件で作り、魚・依頼・所持金・食事効果などの状態差を画面品質の差として扱わない。証拠の保存先と採否判断は `docs/qa/harbor_qa.md` を正とする。
...
採用条件を満たし、3時間帯すべての1280x720実スクショでP1ゼロ、現行beforeへの全画面優位、縮小比較での主CTAと最優先魚の先読みを確認済み。`docs/qa/harbor_qa.md` のfreeze表も本仕様へ更新済み。
