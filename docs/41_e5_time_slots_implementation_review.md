...
作成日: 2026-07-08  
状態: レビュー完了（改善スライス未着手）  
関連: `docs/v2/E5_time_slots.md`（実装仕様） / `docs/qa/harbor_qa.md` / `docs/qa/fishing_surface_qa.md` / `docs/qa/underwater_fight_qa.md`
...
- `docs/qa/fishing_surface_qa.md` — 晴天状態別プレート・非晴天魚影・rain/fog overlay は freeze。**背景 PNG 差し替えは現時点スコープ外**と明記済み
...
| 順 | スライス | 内容 | 依存 |
|---|---|---|---|
| 1 | E5 軽微修正 | ロック文言・監査 BGM ケース | なし |
| 2 | 港画面再構成 | §2 の「出港プラン」カード化 | ui-screen-build / uplift |
| 3 | 釣行 Stage 1 | グレード拡張 + ビネット + visual QA 判定 | なし（素材ゼロ） |
| 4 | 釣行 Stage 2 | 最小素材 4 枚 + 配線 + freeze 改定 | Stage 1 不合格記録 |
