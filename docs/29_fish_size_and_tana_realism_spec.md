...
docs/archive/27_retention_expansion_plan.md §E2「ヌシシステム」（各釣り場に1体の隠れヌシ）で採用する原則。**スライスAの「現実化」はヌシに適用しない**ためにここに明記する。Codexはこの節を実装しないこと（E2着手時に別指示書で扱う）。
...
- ヌシのサイズは通常個体の `size_max` を必ず超えるようにし、図鑑の金枠バッジ＋ヌシ記録併記（docs/archive/27_retention_expansion_plan.md 決定済み）と整合させる
