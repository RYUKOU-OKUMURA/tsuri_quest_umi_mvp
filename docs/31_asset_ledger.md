...
`tools/generate_*.py`（PIL使用・外部AI API不使用・決定的生成）で生成した素材は**プロジェクト所有**であり第三者ライセンスの問題はない。対象: `assets/showcase/cooking/`・`fish_book/`・`fish_market/`・`fishing_spots/`・`harbor/`・`surface/`・`tackle_shop/`・`title/` などの各 generate スクリプト出力一式。
...
| `tools/generate_shark_fish_assets.py` | `assets/showcase/fish/` の E4サメ9種 + `nushi_danger_reef`（PILプロシージャル生成） |
...
- 2026-07-06: E4サメ9種 + `nushi_danger_reef` の魚素材を `tools/generate_shark_fish_assets.py` によるPILプロシージャル生成物として追加
