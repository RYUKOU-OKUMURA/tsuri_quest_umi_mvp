...
| `tools/process_fight_a1_floating_card.py --create-source` | `tools/source_assets/underwater/fight_a1_floating_card_frame_source.png` → `assets/showcase/underwater/fight_floating_card_frame.png`（2026-07-17、Codex作成・プロジェクト所有の純PIL決定的生成。外部画像入力なし。文字・魚・動的値なしのFIGHT専用羊皮紙/濃紺帯/細金縁） |
...
| `tackle_shop/t1_marlin_detail_source.png` | `tools/process_tackle_shop_t1_marlin.py`（`--check` / `--self-test`対応） | `assets/showcase/tackle_shop/shop_detail_item_sheet.png` のmarlin index 4セルだけ（2026-07-18 TACKLE-T1 pilot採用。index 0–3/5–10のdecoded RGBAは不変） | OpenAI built-in image generation（2026-07-18）。文字・数字・UIなしの高解像度竿＋両軸リールsource。OpenAI Terms上のOutput帰属の一般条件のみ確認。個別入力権利・第三者権利clearanceはU-08待ち |
...
| 現行船着き場製品capture | `tools/build_shipyard_d0_proposal.py`（提案生成） / `tools/build_shipyard_visual_comparison.py`（QA比較・再生成） | `reference/shipyard_d0_proposal_unapproved.png`（2026-07-18、作者: Codex、PILによる決定的programmatic再構成、製品非同梱・未採用） | 現行製品captureの画素を提案用referenceへ再配置したQA-only成果物。製品runtime・assetへは未使用。入力capture由来の権利clearanceはU-08待ち |
...
| `cooking/c2_meal_scene_bg_source.png` | `process_cooking_c2_candidate.py`（QA候補） / `process_cooking_c2_product.py`（採用製品の単一writer） | `tools/source_assets/cooking/c2_meal_scene_bg_candidate.png`（QA中間物） / `assets/showcase/cooking/meal_scene_bg.png`（2026-07-17採用。1280×720、食卓・ランタン・港の夕景窓を持つ環境背景） | OpenAI built-in image generationによるsource。`reference/cooking_flow/02_meal_result_concept.png`、現行runtime capture、旧 `meal_scene_bg.png` は方向性/safe-area参照として生成入力に使用し、processorは生成sourceだけを消費。入力権利・第三者権利clearanceはU-08待ち |
| `cooking/c3_exp_burst_source.png` | `tools/cooking_c3_product.py` | `assets/showcase/cooking/exp_burst_frame.png`（2026-07-18 C3採用、760×220 RGBA、中央の暖色放射光・青緑アクセント・粒子だけ（文字/数値/UI/料理/魚なし）） | OpenAI built-in image generationで作成した文字なしsource（参照03は方向性比較だけに使用）、processorはsourceだけを決定的に縮小・透過整形し、入力権利・第三者権利clearanceはU-08待ち |
...
| `status/status_r5b_shell_source.png` | `process_status_r5b_assets.py` | `assets/showcase/status/status_panel_frame.png`, `status_dark_frame.png`, `status_screen_shell.png`（2026-07-17採用。木・真鍮・羊皮紙/濃紺のscreen-local 9-sliceと外周shell。文字・画面部品・魚なし） | OpenAI built-in image generationによるsource。`reference/08_status_screen_mockup.png`は木・真鍮・紙の素材階層だけをスタイル参照として使用し、processorは生成sourceだけを決定的加工。入力権利・第三者権利clearanceはU-08待ち |
...
| 調理C2 食事シーン背景（採用製品） | `tools/source_assets/cooking/c2_meal_scene_bg_source.png` → `process_cooking_c2_candidate.py`によるQA中間物 → `process_cooking_c2_product.py` → `assets/showcase/cooking/meal_scene_bg.png` | OpenAI built-in image generation（2026-07-14）。`docs/51_cooking_c2_meal_scene_asset_brief.md` の文字なし・人物なし・料理なしプロンプトで生成し、2026-07-17にruntime同一状態比較で採用。正式参照、現行runtime、旧背景は方向性/safe-area入力に使用し、product processorはsourceだけを決定的加工 | OpenAI Terms上のOutput帰属という一般条件のみ確認。製品利用中。入力権利・出力の第三者権利clearanceはU-08待ち | 未完 |
| 調理C3 EXP_GAIN祝祭光背（採用製品） | `tools/source_assets/cooking/c3_exp_burst_source.png` → `tools/cooking_c3_product.py` → `assets/showcase/cooking/exp_burst_frame.png` | OpenAI built-in image generation（2026-07-18）。文字・数値・UI・料理・魚なしで生成し、参照03に対する原寸/320×180比較で現行PIL黄青ストライプより明確に祝祭性が増したため、EXP_GAINのTop1光背slotだけへ採用 | OpenAI Terms上のOutput帰属という一般条件のみ確認。入力権利・第三者権利clearanceはU-08待ち | 未完 |
...
| ステータスR5-B 木・真鍮・紙screen shell | `tools/source_assets/status/status_r5b_shell_source.png` → `assets/showcase/status/status_panel_frame.png` / `status_dark_frame.png` / `status_screen_shell.png` | OpenAI built-in image generation（2026-07-17）。正式参照を木・真鍮・羊皮紙の素材階層参照として使用。sourceは文字・画面部品・魚なし、processorは固定crop/9-slice/濃紺grade/alpha matte除去だけを行う | OpenAI Terms上のOutput帰属という一般条件のみ確認。参照入力の権利・出力の第三者権利clearanceはU-08待ち | 未完 |
| 水中FIGHT-A2 下段140pxスリムバー | `tools/source_assets/underwater/fight_a2_slim_bar_frame_source.png` → `assets/showcase/underwater/fight_slim_bar_frame.png` | OpenAI built-in image generation（2026-07-17）。文字・数字・ゲージ・魚・アイコン・キー表記なしの専用プロンプトから生成し、`tools/process_fight_a2_slim_bar.py` は生成sourceだけを固定しきい値crop・1280×140縮小・軽いgradeで製品化。reference画素とruntime画素はprocessor入力に使わない | OpenAI Terms上のOutput帰属という一般条件のみ確認。第三者画像入力なし。出力の第三者権利clearanceはU-08待ち | 未完 |
| TACKLE-T1 marlin 詳細大絵 | `tools/source_assets/tackle_shop/t1_marlin_detail_source.png` → `tools/process_tackle_shop_t1_marlin.py` → `assets/showcase/tackle_shop/shop_detail_item_sheet.png` index 4 | OpenAI built-in image generation（2026-07-18）。文字・数字・UIなしの竿＋両軸リールsourceを、チェッカー背景除去・alpha bbox・384×224 deterministic processorで整形。`reference/`画素は製品processorへ入力していない | OpenAI Terms上のOutput帰属という一般条件のみ確認。生成sourceの個別入力権利・第三者権利clearanceはU-08待ち | 未完 |
...
- 2026-07-17: 調理C2 `meal_scene_bg.png` をruntime同一状態比較で採用。専用product processor、旧一括generator guard、production read-only check、隔離破損self-testへ所有を移し、U-08待ちは維持
- 2026-07-14: ステータスR5-Aの海釣り人sourceをOpenAI built-in image generationで生成し、決定的processorで円形portraitへ整形
- 2026-07-17: FIGHT-A1の文字なしfloating-card専用source/productを純PIL決定的processorで追加。外部画像入力なし、decoded pixels同値時の既存bytes保持とatomic replaceを採用
