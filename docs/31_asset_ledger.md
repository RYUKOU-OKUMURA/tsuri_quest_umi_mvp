...
| `status/status_r5b_shell_source.png` | `process_status_r5b_assets.py` | `assets/showcase/status/status_panel_frame.png`, `status_dark_frame.png`, `status_screen_shell.png`（2026-07-17採用。木・真鍮・羊皮紙/濃紺のscreen-local 9-sliceと外周shell。文字/UI/魚なし） | OpenAI built-in image generationによるsource。`reference/08_status_screen_mockup.png`は木・真鍮・紙の素材階層だけをスタイル参照として使用し、processorは生成sourceだけを決定的加工。入力権利・第三者権利clearanceはU-08待ち |
